...
{
    public class PatientBillPayment
    {
        ...
        public PatientBiillInfoList GettingPatientList(PatientBillList PatientBillList)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(strCompanyID.Trim()))
                {
                    ...
                    if (dtCompanyblock.Rows.Count > 0)
                    {
                ...
                if (DTTemLoadOrders != null)
                {
                    ...
                    if (BillSummary != null)
                    {
                        ...
                        {
                            dtSummaryTable = null; TotAmount = null;
                            ...
                            #region for check Depost Equal to Balance Amount
                            if (System.Configuration.ConfigurationManager.AppSettings["DepositMandatory"] != null && System.Configuration.ConfigurationManager.AppSettings["DepositMandatory"].ToString().ToUpper() == "YES")
                            {
                                TotalBalanceAomunt = Convert.ToDecimal(TotalBalwithoutDeposit);
                                if (hdnDepositAmount >= TotalBalanceAomunt)
                                {
                                ...
                                else
                                {
                                    ...
                                    objPatientList.Code = (int)ProcessStatus.Success;
                                    objPatientList.Status = ProcessStatus.Success.ToString();
                                    objPatientList.Message = "Bill Can't be Generated , Balance Amount : " + TotalBalanceAomuntDeposit + " (SAR)  Need to Pay, Please do the Deposit";
                                    objPatientList.Message2L = "Bill Can't be Generated , Balance Amount : " + TotalBalanceAomuntDeposit + " (SAR)  Need to Pay, Please do the Deposit";
                                    return objPatientList;
                                }
                            }
                            ...
                            if (!string.IsNullOrEmpty(StrFinalBillNo))
                            {
                                ...
                                if (BillSummary != null)
                                {
                                    DtBillSummary = (DataTable)BillSummary.Copy();
                                    DtBillSummary.AcceptChanges();
                                    DtBillSummary.Rows[6]["Amount"] = ActualPatientAmount.ToString();
                                    DtBillSummary.Rows[7]["Amount"] = Convert.ToDecimal(DtBillSummary.Rows[7]["Amount"]) - Convert.ToDecimal(ActualPatientAmount.ToString());
                                }
                                decimal dbval = 0;
                                foreach (DataRow dr in DtBillSummary.Rows)
                                {
                                    dr["Amount"] = Convert.ToDecimal(dr["Amount"]).ToString(hdnsCurrencyFormat);
                                    dr.EndEdit();
                                    DtBillSummary.AcceptChanges();
                                }
                                ...
                                if (objPatientList.BillSummary.Count > 0)
                                {
                                    objPatientList.Code = (int)ProcessStatus.Success;
                                    objPatientList.Status = ProcessStatus.Success.ToString();
                                    objPatientList.Message = strbuilderBillNo.ToString();
                            ...
                            else
                            {
                                ...
                                //objPatientList.Message2L = Resources.Arabic.ResourceManager.GetString("FAILGBILL");
                                objPatientList.Message = System.Configuration.ConfigurationManager.AppSettings["FAILGBILL"].ToString();
                                objPatientList.Message2L = System.Configuration.ConfigurationManager.AppSettings["FAILGBILL"].ToString();
                            }
                        }
                        else
                        {
        ...
        {
            decimal cash = 0; string StrFBLNO = string.Empty;
            try
            {

                string strRoleCheck = ConfigurationSettings.AppSettings["PatientOutstandingAproval"].ToString();
                int PatientOutstandingAmtM = 0;
                if (PatientOutstandingAmt > 0)
                    PatientOutstandingAmtM = Convert.ToInt32(PatientOutstandingAmt);

                DataTable DtCreditBillDetails = null;
                DataTable dtCompanyCashBLPAckageItems = null;
                DataTable dtCashContributions = null;
                if (dtSummary != null)
                {
                    DtCreditBillDetails = (DataTable)dtSummary.Copy();
                    DtCreditBillDetails.AcceptChanges();
                }
                if (ViewStatedtCompanyCashContribution != null)
                {
                    dtCashContributions = (DataTable)ViewStatedtCompanyCashContribution.Copy();
                    dtCashContributions.AcceptChanges();
                }
                DtDiscountDetails = new DataTable();
                if (dtPackageItems != null)
                {
                    dtCompanyCashBLPAckageItems = (DataTable)dtPackageItems.Copy();
                    dtCompanyCashBLPAckageItems.AcceptChanges();
                }

                string strBillAmount = string.Format("{0:F}", Convert.ToDecimal(DtCreditBillDetails.Compute("Sum([Amount])", "")));
                string strPatientShare = string.Format("{0:F}", (DtCreditBillDetails.Compute("Sum([SPay])+Sum([PPay])", "")));

                cash = Convert.ToDecimal(txtamount);

                #region Multimode 
                if (dtSummaryTable == null)
                {
                    btnMOPOK_Click();
                    if (intMOPoutstand == 1)
                        return StrFBLNO;
                ...
                decimal dAmount = Convert.ToDecimal(txtamount);
                BlanceAomunt = (Decimal)BAmount;
                decimal dOutStandAmount = Convert.ToDecimal(BlanceAomunt);
                string AmtRemarks = ConfigurationSettings.AppSettings["AmountRemarks"].ToLower();
                if (dAmount > dOutStandAmount)
                {
                    return StrFBLNO;
                }
                else if ((TotAmount != null) && (AmtRemarks == "yes") && (dOutStandAmount > Convert.ToDecimal(TotAmount)))
                {
                    //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','Amount cannot be less than Balance Amount','OK','Information');", true);
                    return StrFBLNO;
                }
                else if ((!string.IsNullOrEmpty(TotAmount)) && (AmtRemarks == "yes") && (dOutStandAmount > Convert.ToDecimal(TotAmount)))
                {
                    string strMessage = string.Empty;

                    StrBillNo = SaveBill(DtCreditBillDetails, dtCompanyCashBLPAckageItems, DtDiscountDetails, dtCashContributions);
                    if (StrBillNo.ToString() == "")
                    {
                        //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','" + NxGWeb.WebUI.Admin.CommonHelper.GENRALERROR + "','OK','Error');", true);
                        return StrFBLNO;
                    }
                    else
                    {
                        txtbillno = StrBillNo.ToString();
                        NewBillNo = StrBillNo.ToString();
                        string PrintBillnumber = BillNo.ToString();
                        string SPBillNumber = string.Empty;
                        string TokenNumber = string.Empty;
                        if (SPBillNO != null && SPBillNO.ToString() != "")
                        {
                            SPBillNumber = SPBillNO.ToString();
                        }
                        if (TokenNO != null && TokenNO.ToString() != "")
                        {
                            TokenNumber = TokenNO.ToString();
                        }
                        txtamount = Convert.ToDecimal("0").ToString(hdnsCurrencyFormat);
                        ScheduleDT = null;
                        StrFBLNO = StrBillNo.ToString();
                        return StrFBLNO;
                    }
        ...
        private string SaveBill(DataTable DtCreditBillDetails, DataTable DtCompanyCashBLPackageItemDetails, DataTable dtDiscountDetails, DataTable dtCompanyCashContribution)
        {
            bool isFirstTimeLOA = false;
            string strFirstTimeLOAID = string.Empty;
            try
            {
                DataSet dsLOA = null;
                DataTable dtBillTable = new DataTable("SaveBillDetails");
                DataSet dsCompCash = new DataSet();
                DataTable dtTempCahsContribution = null;
                DataTable dtTempCreditContribution = null;
                DataTable DtCompanyCreditBLPackageItemDetails = null;
                DataTable dtTempLetter = new DataTable();
                DataSet dsCash = new DataSet();
                DataTable dtletter = new DataTable();
                int MOPId = 1; //cash
                string PayTransNo = "";
                string CardNo = "";
                int BankId = 0;
                ...
                decimal Credit = 0;
                StrBillId = "";
                string StrBLNO = string.Empty;
                int intDupLetterid = 0;
                bool isBothBills;
                int DiscAuthorisedBy = 0;
                string DiscRemarks = "";
                try
                {
                    #region Populating BillTable
                    string strBillNo = "";
                    string strBillCrNo = "";
                    string[] StrCredit = null;
                    string[] StrCs = null;
                    string[] strtoken = null;
                    int intHasDefaultOPPackage = 0;
                    if (intPackageID > 0)
                    {
                        intHasDefaultOPPackage = 0;
                    }

                    dtBillTable.Columns.Add("BillID", typeof(int));
                    dtBillTable.Columns.Add("BillNo", typeof(string));
                    dtBillTable.Columns.Add("VisitNumber", typeof(string));
                    dtBillTable.Columns.Add("Billdate", typeof(string));
                    dtBillTable.Columns.Add("PatientType", typeof(int));
                    dtBillTable.Columns.Add("PatientId", typeof(int));
                    dtBillTable.Columns.Add("IPID", typeof(int));
                    dtBillTable.Columns.Add("TitleID", typeof(int));
                    dtBillTable.Columns.Add("FirstName", typeof(string));
                    dtBillTable.Columns.Add("MiddleName", typeof(string));
                    dtBillTable.Columns.Add("LastName", typeof(string));
                    dtBillTable.Columns.Add("GenderID", typeof(int));
                    dtBillTable.Columns.Add("Age", typeof(int));
                    dtBillTable.Columns.Add("AgeUOMID", typeof(int));
                    dtBillTable.Columns.Add("DoctorID", typeof(int));
                    dtBillTable.Columns.Add("RefDoctorId", typeof(int));
                    dtBillTable.Columns.Add("CompanyId", typeof(int));
                    dtBillTable.Columns.Add("TariffId", typeof(int));
                    dtBillTable.Columns.Add("EmpId", typeof(int));
                    dtBillTable.Columns.Add("Amount", typeof(decimal));
                    dtBillTable.Columns.Add("Cash", typeof(decimal));
                    dtBillTable.Columns.Add("DepositAvailed", typeof(decimal));
                    dtBillTable.Columns.Add("Discount", typeof(string));
                    dtBillTable.Columns.Add("Credit", typeof(decimal));
                    dtBillTable.Columns.Add("PaymentModeID", typeof(int));
                    dtBillTable.Columns.Add("PayTranNumber", typeof(string));
                    dtBillTable.Columns.Add("CardNumber", typeof(string));
                    dtBillTable.Columns.Add("BankID", typeof(int));
                    dtBillTable.Columns.Add("CardDate", typeof(string));
                    dtBillTable.Columns.Add("CardId", typeof(int));
                    dtBillTable.Columns.Add("IssueDate", typeof(string));
                    dtBillTable.Columns.Add("Remarks", typeof(string));
                    dtBillTable.Columns.Add("ParentBillID", typeof(int));
                    dtBillTable.Columns.Add("LetterId", typeof(int));
                    dtBillTable.Columns.Add("UserID", typeof(int));
                    dtBillTable.Columns.Add("WorkstationID", typeof(int));
                    dtBillTable.Columns.Add("VisitID", typeof(int));
                    dtBillTable.Columns.Add("VisitTYpe", typeof(int));
                    dtBillTable.Columns.Add("NewVisitID", typeof(int));
                    dtBillTable.Columns.Add("EpisodeID", typeof(int));
                    dtBillTable.Columns.Add("HospitalID", typeof(int));
                    dtBillTable.Columns.Add("CPAY", typeof(decimal));
                    dtBillTable.Columns.Add("PPAY", typeof(decimal));
                    dtBillTable.Columns.Add("SPAY", typeof(decimal));
                    dtBillTable.Columns.Add("DPAY", typeof(decimal));
                    dtBillTable.Columns.Add("GradeID", typeof(int));
                    dtBillTable.Columns.Add("SpecialiseID", typeof(int));
                    dtBillTable.Columns.Add("FirstName2l", typeof(string));
                    dtBillTable.Columns.Add("MiddleName2l", typeof(string));
                    dtBillTable.Columns.Add("LastName2l", typeof(string));
                    dtBillTable.Columns.Add("OldBillID", typeof(int));
                    dtBillTable.Columns.Add("IsMaxCollectable", typeof(int));
                    dtBillTable.Columns.Add("MemberShipTypeId", typeof(int));
                    dtBillTable.Columns.Add("MemberShipTypeCode", typeof(string));
                    dtBillTable.Columns.Add("CollectableType", typeof(int));
                    dtBillTable.Columns.Add("HasDefaultLOA", typeof(int));
                    dtBillTable.Columns.Add("IsXml", typeof(int));
                    dtBillTable.Columns.Add("CollectablePaidBy", typeof(int));
                    dtBillTable.Columns.Add("IsCheckin", typeof(int));
                    dtBillTable.Columns.Add("InsuranceCardNo", typeof(string));
                    dtBillTable.Columns.Add("HasDefaultOPPackage", typeof(int));
                    dtBillTable.Columns.Add("Sessionid", typeof(string));
                    dtBillTable.Columns.Add("ClaimFormNo", typeof(string));
                    dtBillTable.Columns.Add("CashGiven", typeof(decimal));
                    dtBillTable.Columns.Add("IsServicePatient", typeof(int));
                    dtBillTable.Columns.Add("DOB", typeof(string));
                    dtBillTable.Columns.Add("MaritalStatusID", typeof(int));
                    dtBillTable.Columns.Add("Address01", typeof(string));
                    dtBillTable.Columns.Add("Address02", typeof(string));
                    dtBillTable.Columns.Add("PhoneNo", typeof(string));
                    dtBillTable.Columns.Add("MLCtype", typeof(int));
                    dtBillTable.Columns.Add("NationalID", typeof(string));
                    dtBillTable.Columns.Add("FamilyName", typeof(string));
                    dtBillTable.Columns.Add("PayerDocumentID", typeof(string));
                    dtBillTable.Columns.Add("DiscAuthorisedBy", typeof(Int32));
                    dtBillTable.Columns.Add("DiscRemarks", typeof(string));
                    dtBillTable.Columns.Add("DiscountTYpeID", typeof(Int32));


                    if (!string.IsNullOrEmpty(ViewStateLetterid))
                    {
                        string strLetterNo = ViewStateLetterid.ToString();
                        intLetterNo = Convert.ToInt32(strLetterNo);
                    }
                    if (!string.IsNullOrEmpty(VSParentLetterid))
                    {
                        string strParentLetterNo = VSParentLetterid.ToString();
                        ParentLetterId = Convert.ToInt32(strParentLetterNo);
                    }
                    dtTempLetter.Columns.Add("LET", typeof(int));
                    if (intLetterNo > 0)
                    {
                        DataRow drAdd = null;
                        drAdd = dtTempLetter.NewRow();
                        drAdd["LET"] = intLetterNo;
                        dtTempLetter.Rows.Add(drAdd);
                        dtTempLetter.AcceptChanges();
                    }
                    dtTempLetter.TableName = "Letter";
                    #region ModeOfPayment
                    string strCDate = string.Empty;
                    MOPId = 1;//cash
                    if (MOPId == 1)
                    {
                        PayTransNo = "1";
                    ...
                    strCDate = string.Empty;
                    if (!string.IsNullOrEmpty(strCDate))
                        CardDate = Convert.ToDateTime(strCDate).ToString("dd-MMM-yyyy");
                    else
                        CardDate = "";
                    CardId = 0;
                    #endregion
                    ...
                    {
                        string strBillAmtWithVAT = string.Format("{0:F}", Convert.ToDecimal(DtCreditBillDetails.Compute("Sum([Amount])", "")) + Convert.ToDecimal(hdnVATAmount));
                        strBillAmount = strBillAmtWithVAT;
                    }
                    string strPatientShare = string.Format("{0:F}", (DtCreditBillDetails.Compute("Sum([SPay])+Sum([PPay])", "")));
                    string strBalance = strBillAmount;
                    decimal Roundoff = Convert.ToDecimal(strBalance) - Convert.ToDecimal(strBalance);
                    if (OPBAvailDisc != null)
                    {
                        dtDiscountDetails = (DataTable)OPBAvailDisc.Copy();
                        dtDiscountDetails.AcceptChanges();
                    }
                    else if (AvailDiscount != null && hdnrblbilltypeCredit == true)
                        dtDiscountDetails = (DataTable)AvailDiscount.Copy();
                    dtDiscountDetails.AcceptChanges();
                    ...
                    if (BillType == "1")
                    {
                        ...
                        {
                            Discount = Convert.ToDecimal(dtDiscountDetails.Compute("Sum([DIS])", ""));
                            Amount = (Convert.ToDecimal(dtCompanyCashContribution.Compute("Sum([TOT])", "([TYP]=0)")));
                            if (!string.IsNullOrEmpty(hdnVATAmount))
                            {
                                Amount += Convert.ToDecimal(hdnVATAmount);
                            }
                            if (Convert.ToDecimal(txtamount) - (Convert.ToDecimal(strBalance) + Roundoff) == 0)
                            { Cash = Convert.ToDecimal(strBalance); }
                            else
                            { Cash = Convert.ToDecimal(txtamount); }
                            #region Multimode : bhaskar

                            DataTable dtPayment = new DataTable();
                            if (dtSummaryTable != null)
                            {
                                dtPayment = (DataTable)dtSummaryTable;
                                if (dtPayment != null && dtPayment.Rows.Count > 0)
                                {
                                    decimal Tempamt = Convert.ToDecimal(dtPayment.Compute("Sum([Amount])", ""));
                                    if (Convert.ToDecimal(Tempamt) - (Convert.ToDecimal(strBalance) + Roundoff) == 0)
                                    { Cash = Convert.ToDecimal(strBalance); }
                                    else
                                    { Cash = Convert.ToDecimal(Tempamt); }
                                }
                            }
                            #endregion
                        ...
                        if (hdnDepositAmount > 0)
                        {
                            DepositAvailed = Convert.ToDecimal(BillSummary.Rows[4]["Amount"]);
                            Amount = Convert.ToDecimal(BillSummary.Rows[0]["Amount"]);
                        }
                        else
                        {
                            DepositAvailed = 0;
                        }
                        CPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([CPAY])", ""));
                        PPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([SPAY])", ""));
                        SPAY = 0;
                        DPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([DPAY])", ""));
                        if (SPAY > 0)
                        {
                            if (!string.IsNullOrEmpty(hdnVATAmount))
                            {
                                SPAY = SPAY + Convert.ToDecimal(hdnVATAmount);
                            }
                        }
                        if (CPAY > 0)
                        {
                            if (!string.IsNullOrEmpty(hdnVATAmount))
                            {
                                CPAY = CPAY + Convert.ToDecimal(hdnVATAmount);
                            }
                        }
                        if (PPAY > 0)
                        {
                            if (!string.IsNullOrEmpty(hdnVATAmount))
                            {
                                PPAY = PPAY + Convert.ToDecimal(hdnVATAmount);
                            }
                        }
                    }
                    ...
                    {
                        decimal tempBalAmount = 0;
                        CPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([CPAY])", ""));
                        PPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([PPAY])", ""));
                        SPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([SPAY])", ""));
                        DPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([DPAY])", ""));
                        if (!string.IsNullOrEmpty(hdnCPAYAfterVAT))
                        {
                            CPAY = Convert.ToDecimal(hdnCPAYAfterVAT);
                        }
                        else
                        {
                            CPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([CPAY])", ""));
                        }
                        if (!string.IsNullOrEmpty(hdnPPAYAfterVAT))
                        {
                            PPAY = Convert.ToDecimal(hdnPPAYAfterVAT);
                        }
                        else
                        {
                            PPAY = Convert.ToDecimal(DtCreditBillDetails.Compute("sum([PPAY])", ""));
                        ...
                        {
                            Amount = Convert.ToDecimal(strBillAmount);
                            tempBalAmount = Convert.ToDecimal(BillSummary.Rows[7]["Amount"]);
                            decimal tempcond = Convert.ToDecimal(txtamount);
                            DataTable dtPayment = new DataTable();
                            if (dtSummaryTable != null)
                            {
                                dtPayment = (DataTable)dtSummaryTable;
                        ...
                        if (!string.IsNullOrEmpty(hdnCPAYAfterVAT))
                        {
                            Credit = Convert.ToDecimal(hdnCPAYAfterVAT);
                        }
                        else
                        {
                            Credit = Convert.ToDecimal(DtCreditBillDetails.Compute("Sum([CPAY])", ""));
                        }
                        //end

                    }

                    //if (radbtnVisitInfo.Items[0].Selected)
                    //{
                    //    intVisitType = 1;
                    //}
                    //if (!radbtnVisitInfo.Items[0].Selected && !radbtnVisitInfo.Items[1].Selected)
                    //{
                    intVisitType = 1;
                    intVisitId = 0;
                    intNewVisitId = 0;
                    intEpisodeId = 0;
                    //}                    
                    //if (radbtnVisitInfo.Items[1].Selected)
                    //{
                    //    intVisitType = 2;
                    //    intVisitId = Convert.ToInt32(hdnintVisitId.Value);
                    //    intNewVisitId = Convert.ToInt32(hdnintNewVisitId.Value);
                    //    intEpisodeId = Convert.ToInt32(hdnintEpisodeId.Value);
                    //}

                    //  strDiscRemarks = strDiscRemarks;//+ txtremarks.Text.ToString();
                    strDiscRemarks = string.Empty;// + txtremarks.Text.ToString();
                    //DataTable dtPatientDetails = null;
                    DTPatientData = DTPatientDetails();

                    #region VaribaleDeclaration

                    int PatientType, PatientId, IPID, DocId, CompanyId, TariffId, GradeId, MaxCollectable, CollectableType, intUserid, intMarStatusID;
                    int intSpecializationID, intDiscAuthoriedBy, CollectablePaidBy, intIsCheckin, intAge, intGenderID, intRefdocID, intAgeUomid, intTitleid, intFacilityid, intHospitalid;
                    string strInsuranceNo = string.Empty, strAddr1 = string.Empty, strAddr2 = string.Empty, strPhno = string.Empty, strNationalID = string.Empty, strDOB = string.Empty;
                    string strFamilyName = string.Empty;
                    int intMLCtype;
                    #endregion VaribaleDeclaration

                    if (!string.IsNullOrEmpty(hdnPatientType))
                        PatientType = Convert.ToInt32(hdnPatientType);
                    else
                        PatientType = 1;
                    if (!string.IsNullOrEmpty(hdnPatientID))
                        PatientId = Convert.ToInt32(hdnPatientID);
                    else
                        PatientId = 0;
                    if (!string.IsNullOrEmpty(hdnIPID))
                        IPID = Convert.ToInt32(hdnIPID);
                    else
                        IPID = 0;

                    if (!string.IsNullOrEmpty(hdnDocID))
                        DocId = Convert.ToInt32(hdnDocID);
                    else
                        DocId = 0;
                    ...
                    if (!string.IsNullOrEmpty(hdnTariffID))
                        TariffId = Convert.ToInt32(hdnTariffID);
                    else
                        TariffId = -1;
                    if (!string.IsNullOrEmpty(strGradeID))
                        GradeId = Convert.ToInt32(strGradeID);
                    else
                        GradeId = 0;
                    if (!string.IsNullOrEmpty(hdnCollectableType))
                        CollectableType = Convert.ToInt32(hdnCollectableType);
                    else
                        CollectableType = 0;

                    DTTemp = (DataTable)DTTem.Copy();
                    DTTemp.AcceptChanges();
                    if (DTTemp != null && DTTemp.Rows.Count > 0 && DTTemp.Select("ServiceId=2").Count() > 0)
                    {
                        DataRow[] drDoct = DTTemp.Select("ServiceId=2");
                        intSpecializationID = Convert.ToInt32(drDoct[0]["SpecialiseId"]);
                    }
                    else if (!string.IsNullOrEmpty(hdnDocSpecialiseId))
                        intSpecializationID = Convert.ToInt32(hdnDocSpecialiseId);
                    else
                        intSpecializationID = 0;
                    //if (!string.IsNullOrEmpty(hdnDiscAuthorisedBy))
                    //{
                    //    string[] strAuthorizedBy = hdnDiscAuthorisedBy.Split(',');
                    //    string[] strAid = strAuthorizedBy[2].ToString().Split(':');
                    //    intDiscAuthoriedBy = Convert.ToInt32(strAid[1]);
                    //}

                    //else
                    intDiscAuthoriedBy = 0;
                    //if (!string.IsNullOrEmpty(hdnCollectablePaidBy))
                    //    CollectablePaidBy = Convert.ToInt32(hdnCollectablePaidBy);
                    //else
                    CollectablePaidBy = 0;
                    //if (!string.IsNullOrEmpty(hndIsCheckin))
                    //    intIsCheckin = Convert.ToInt32(hndIsCheckin);
                    //else
                    intIsCheckin = 0;
                    //if (!string.IsNullOrEmpty(hdnInsuranceNo))
                    //    strInsuranceNo = hdnInsuranceNo;
                    //else
                    strInsuranceNo = "";
                    if (!string.IsNullOrEmpty(hdnMaxCollectable))
                        MaxCollectable = Convert.ToInt32(hdnMaxCollectable);
                    else
                        MaxCollectable = 0;
                    if (!string.IsNullOrEmpty(hdnAge))
                        intAge = Convert.ToInt32(hdnAge);
                    else
                        intAge = 0;

                    if (!string.IsNullOrEmpty(hdnGenderID))
                        intGenderID = Convert.ToInt32(hdnGenderID);
                    else
                        intGenderID = 0;
                    if (!string.IsNullOrEmpty(hdAgeUomid))
                        intAgeUomid = Convert.ToInt32(hdAgeUomid);
                    else
                        intAgeUomid = 0;
                    if (!string.IsNullOrEmpty(hdnTitleID))
                        intTitleid = Convert.ToInt32(hdnTitleID);
                    else
                        intTitleid = 0;
                    ...
                    if (!string.IsNullOrEmpty(hdnMarStatusID))
                        intMarStatusID = Convert.ToInt32(hdnMarStatusID);
                    else intMarStatusID = 0;
                    if (!string.IsNullOrEmpty(hdnDOB))
                        strDOB = hdnDOB.ToString();
                    else strDOB = string.Empty;

                    if (!string.IsNullOrEmpty(hdnPhno))
                        strPhno = hdnPhno.ToString();
                    else strPhno = string.Empty;

                    if (!string.IsNullOrEmpty(hdnNationalID))
                        strNationalID = hdnNationalID.ToString();
                    else strNationalID = string.Empty;

                    if (!string.IsNullOrEmpty(hdnFamilyName))
                    ...
                    // if (txtActualAMT.Text != null && txtActualAMT.Text != string.Empty)
                    // decCashGiven = Convert.ToDecimal(txtActualAMT.Text);

                    if (dtSummaryTable != null)
                    {
                        DataTable dtPayment = new DataTable();
                        dtPayment = (DataTable)dtSummaryTable;
                        if (dtPayment != null && dtPayment.Rows.Count > 1)
                        {
                            //decCashGiven = Cash;
                            //if (txtActualAMT.Text != null && txtActualAMT.Text != string.Empty)
                            //    decCashGiven = Convert.ToDecimal(txtActualAMT.Text);
                            //else
                            //{
                            DataRow[] drcash = dtPayment.Select("Modeid=1");
                            if (drcash.Length > 0)
                            {
                                decCashGiven = Convert.ToDecimal(drcash[0]["Amount"]);
                            }
                            //}
                            bitXml = 1;
                        }
                        else if (dtPayment != null && dtPayment.Rows.Count == 1)
                        {
                            //if (txtActualAMT.Text != null && txtActualAMT.Text != string.Empty)
                            //    decCashGiven = Convert.ToDecimal(txtActualAMT.Text);
                            //else
                    ...
                    else
                    {
                        ...
                        //  decCashGiven = decAmount;
                        // }
                    }

                    #region Multimode : bhaskar
                    int intMembershipTypeID = 0;
                    string strMembershipCode = string.Empty;
                    //if (ddlMembershipType.Items.Count > 1)
                    //    intMembershipTypeID = Convert.ToInt32(ddlMembershipType.SelectedValue);
                    //strMembershipCode = txtMembershipcode.Text;
                    ...
                    {
                        DiscAuthorisedBy = Convert.ToInt32(dtDiscountDetails.Rows[0]["AuthorisedId"].ToString());
                        DiscRemarks = dtDiscountDetails.Rows[0]["Remarks"].ToString();
                        if (dtDiscountDetails.Columns.Contains("DiscountTypeID"))
                            DiscountTypeID = Convert.ToInt32(dtDiscountDetails.Rows[0]["DiscountTypeID"]);
                        else if (!string.IsNullOrEmpty(hdnDiscountID))
                            DiscountTypeID = Convert.ToInt32(hdnDiscountID);
                    }
                    //   Lanka VAT CR
                    dtBillTable.Columns.Add("PVAT", typeof(decimal));
                    dtBillTable.Columns.Add("CVAT", typeof(decimal));
                    dtBillTable.Columns.Add("POSTransactionNumber");
                    decimal VATAmount = 0;
                    if (!string.IsNullOrEmpty(hdnVATAmount))
                    {
                        VATAmount = Convert.ToDecimal(hdnVATAmount);
                    }
                    decimal pvatamount = 0;
                    decimal cvatamount = 0;
                    if (!string.IsNullOrEmpty(hdnCVATValue))
                    {
                        cvatamount = Convert.ToDecimal(hdnCVATValue);
                    }
                    if (!string.IsNullOrEmpty(hdnPVATValue))
                    {
                        pvatamount = Convert.ToDecimal(hdnPVATValue);
                    }

                    dtBillTable.Columns.Add("PolicyNo", typeof(string));
                    dtBillTable.Columns.Add("MRNo", typeof(string));
                    dtBillTable.Columns.Add("ScanDocumentPath", typeof(string));
                    dtBillTable.Columns.Add("InsuranceNo", typeof(string));
                    dtBillTable.Columns.Add("CardExpirydate", typeof(DateTime));

                    DataTable dtpatient01 = (DataTable)dtPatientDetails.Copy();




                    string strPolicyNo, strMRNo, strScanDocumentPath, striInsuranceNo, strCardExpiry;
                    strPolicyNo = strMRNo = strScanDocumentPath = striInsuranceNo = string.Empty;
                    strCardExpiry = "01-Jan-1990 00:00:00";
                    if (!string.IsNullOrEmpty(dtpatient01.Rows[0]["PolicyNo"].ToString()))
                        strPolicyNo = dtpatient01.Rows[0]["PolicyNo"].ToString();
                    if (!string.IsNullOrEmpty(dtpatient01.Rows[0]["MRNo"].ToString()))
                        strMRNo = dtpatient01.Rows[0]["MRNo"].ToString();
                    if (!string.IsNullOrEmpty(dtpatient01.Rows[0]["ScanDocumentPath"].ToString()))
                        strScanDocumentPath = dtpatient01.Rows[0]["ScanDocumentPath"].ToString();
                    if (!string.IsNullOrEmpty(dtpatient01.Rows[0]["InsuranceNo"].ToString()))
                        striInsuranceNo = dtpatient01.Rows[0]["InsuranceNo"].ToString();
                    if (!string.IsNullOrEmpty(dtpatient01.Rows[0]["CardExpirydate"].ToString()))
                        strCardExpiry = dtpatient01.Rows[0]["CardExpirydate"].ToString();

                    if (bitXml == 0)
                    {
                        dtBillTable.Rows.Add(new object[] {0,"","0",DBNull.Value,PatientType,PatientId,IPID,intTitleid,lblFirstName,lblMiddleName,lblLastName,
                                                               Convert.ToInt32(intGenderID),Convert.ToInt32(intAge),intAgeUomid ,
                                                               Convert.ToInt32(DocId),Convert.ToInt32(intRefdocID),CompanyId,
                                                               TariffId,
                                                               Convert.ToInt32(intDiscAuthoriedBy),Convert.ToDecimal(Amount),Convert.ToDecimal(Cash),Convert.ToDecimal(DepositAvailed),
                                                               Convert.ToString(Discount),Convert.ToDecimal(Credit),Convert.ToInt32(MOPId),Convert.ToString(PayTransNo),Convert.ToString(CardNo),Convert.ToInt32(BankId),Convert.ToString(CardDate),Convert.ToInt32(CardId),Convert.ToString(IssueDatetime),
                                                               strDiscRemarks,1,Convert.ToInt32(intLetterNo),intUserid,intFacilityid,Convert.ToInt32(intVisitId),Convert.ToInt32(intVisitType),
                                                               intNewVisitId,intEpisodeId,intHospitalid,Convert.ToDecimal(CPAY),
                                                               Convert.ToDecimal(PPAY),Convert.ToDecimal(SPAY),
                                                               Convert.ToDecimal(DPAY), Convert.ToInt32(GradeId), Convert.ToInt32(intSpecializationID),
                                                               hdnFirstName2L.ToString(),hdnMiddleName2L.ToString(),hdnLastName2L.ToString(),
                            0,Convert.ToInt32(MaxCollectable>0? 1:0),intMembershipTypeID,strMembershipCode,Convert.ToInt32(CollectableType),0,0,Convert.ToInt32(CollectablePaidBy),Convert.ToInt32(intIsCheckin),strInsuranceNo,Convert.ToInt32(intHasDefaultOPPackage),Sessionid,ClaimFormNo,decCashGiven,Convert.ToInt32(hdnIsDirectPatient),
                                                               strDOB,intMarStatusID,strAddr1,strAddr2,strPhno,Convert.ToInt32(intMLCtype),strNationalID,
                                                               strFamilyName,"",
                                                               DiscAuthorisedBy,DiscRemarks,DiscountTypeID == 0?null:DiscountTypeID.ToString(),pvatamount,cvatamount,hdnPOSTransactionNumber,strPolicyNo, strMRNo, strScanDocumentPath, striInsuranceNo,strCardExpiry});
                    }
                    else
                    {
                        dtBillTable.Rows.Add(new object[] {0,"","",DBNull.Value,PatientType,PatientId,IPID,intTitleid,lblFirstName,lblMiddleName,lblLastName,
                                                               Convert.ToInt32(intGenderID),Convert.ToInt32(intAge),intAgeUomid,
                                                               Convert.ToInt32(DocId),Convert.ToInt32(intRefdocID),Convert.ToInt32(CompanyId),
                                                               Convert.ToInt32(TariffId),
                                                               Convert.ToInt32(intDiscAuthoriedBy),Convert.ToDecimal(Amount),Convert.ToDecimal(Cash),Convert.ToDecimal(DepositAvailed),
															  // Convert.ToString(Discount),Convert.ToDecimal(Credit),null,null,null,null,null,null,null,
                                                              Convert.ToString(Discount),Convert.ToDecimal(Credit),0,null,null,0,null,0,null,
                                                               strDiscRemarks,1,Convert.ToInt32(intLetterNo),intUserid,intFacilityid,Convert.ToInt32(intVisitId),Convert.ToInt32(intVisitType),
                                                               intNewVisitId,intEpisodeId,intHospitalid,Convert.ToDecimal(CPAY),
                                                               Convert.ToDecimal(PPAY),Convert.ToDecimal(SPAY),
                                                               Convert.ToDecimal(DPAY),Convert.ToInt32(GradeId), Convert.ToInt32(intSpecializationID),
                                                               hdnFirstName2L.ToString(),hdnMiddleName2L.ToString(),hdnLastName2L.ToString(),
                            0,Convert.ToInt32(MaxCollectable>0? 1:0),intMembershipTypeID,strMembershipCode,Convert.ToInt32(CollectableType),Convert.ToInt32(0),Convert.ToInt32(1),Convert.ToInt32(CollectablePaidBy),Convert.ToInt32(intIsCheckin),strInsuranceNo,Convert.ToInt32(intHasDefaultOPPackage),Sessionid,ClaimFormNo,decCashGiven,Convert.ToInt32(hdnIsDirectPatient),
                                                               strDOB,intMarStatusID,strAddr1,strAddr2,strPhno,Convert.ToInt32(intMLCtype),strNationalID,
                                                               strFamilyName,"",
                                                               DiscAuthorisedBy,DiscRemarks,DiscountTypeID == 0?null:DiscountTypeID.ToString(),pvatamount,cvatamount,hdnPOSTransactionNumber,strPolicyNo, strMRNo, strScanDocumentPath, striInsuranceNo,strCardExpiry});
                    }
                    ...
                    if (hdnblnSaveDefaultLOA == "true" && hdnrblbilltypeCredit == true)
                    {


                        if (ViewStatedsLOA != null)
                        {
                            dsLOA = (DataSet)ViewStatedsLOA.Copy();
                            dsLOA.AcceptChanges();
                            dtLetterSeventh = dsLOA.Tables[1].Copy();
                            dtLetterGenInfo = dsLOA.Tables[0].Copy();
                        }

                        if (dtLetterGenInfo.Rows.Count > 0)
                        {
                            DTTemp = (DataTable)DTTem.Copy();
                            DTTemp.AcceptChanges();
                            DataRow[] drDoct = DTTemp.Select("ServiceId=2");
                            int intBillDocSpec = 0;

                            CultureInfo ci = new CultureInfo("en-US");
                            string strDatestr = (System.DateTime.Now.AddSeconds((-45))).ToString("dd-MMM-yyyy hh:mm:ss tt");

                            if (drDoct.Length > 0)
                            {
                                if (Convert.ToInt32(drDoct[0]["SampleId"]) == Convert.ToInt32(FetchDefaultTariff("ORD_FOLLOWUP_DAYS")))
                                {
                                    intBillDocSpec = Convert.ToInt32(drDoct[0]["SpecialiseId"]);
                                    dtletter = FetchADVProcedureDetails("FetchPatientLettersAdv", "0", "Blocked=0 and patientid=" + PatientId + " and SpecialisationId= " + intBillDocSpec + " and CompanyId = " + Convert.ToInt32(strCompanyID) + " and Gradeid = " + Convert.ToInt32(strGradeID) + " and TODate >= '" + strDatestr + "' and datediff(day,TODate,getdate())<" + hdnLOAfollowupDays, intUserid, intFacilityid, 0);//System.DateTime.Now.AddSeconds((-45))
                                    intSpecializationID = intBillDocSpec;
                            ...
                            {
                                dtletter = FetchADVProcedureDetails("FetchPatientLettersAdv", "0", "Blocked=0 and patientid=" + PatientId + " and SpecialisationId= " + intSpecializationID + " and CompanyId = " + Convert.ToInt32(strCompanyID) + " and Gradeid = " + Convert.ToInt32(strGradeID) + " and FromDate >= '" + strDatestr + "'", intUserid, intFacilityid, 0);//System.DateTime.Now.AddSeconds((-45))
                            }
                            if (dtletter != null && dtletter.Rows.Count > 0)
                            {
                                if (dtTempLetter.Rows.Count > 0)
                                {
                                    dtTempLetter.Rows[0]["LET"] = dtletter.Rows[0]["LetterId"];
                                    dtTempLetter.AcceptChanges();
                                }
                            }
                            else
                            {
                                ...
                                FillAgreement(dtLetterSeventh.Copy());
                                if (string.IsNullOrEmpty(AlreadySavedLetterID))
                                {
                                    SaveDefaultLOA(dtLetterSeventh, dtLetterGenInfo, PatientId.ToString(), intSpecializationID, "DefaultLOA");
                                    isFirstTimeLOA = true;
                                }

                                if (dtTempLetter.Rows.Count > 0)
                                {
                                    string strDatestrSecond = (System.DateTime.Now).ToString("dd-MMM-yyyy");
                                    string stFilter = "Blocked=0 and patientid=" + PatientId + " and SpecialisationId= " + intSpecializationID + " and CompanyId = " + Convert.ToInt32(strCompanyID) + " and Gradeid = " + Convert.ToInt32(strGradeID) + " and FromDate >= '" + strDatestrSecond + "'";//System.DateTime.Now.Date 

                                    DataTable dtlett = FetchADVProcedureDetails("FetchPatientLettersAdv", "0", stFilter, intUserid, intFacilityid, 0);

                                    if (dtlett.Rows.Count > 0)
                                    {
                                        dtTempLetter.Rows[0]["LET"] = dtlett.Rows[dtlett.Rows.Count - 1]["LetterId"];
                                        AlreadySavedLetterID = Convert.ToInt32(dtlett.Rows[dtlett.Rows.Count - 1]["LetterId"]).ToString();
                                        dtTempLetter.AcceptChanges();
                                        strFirstTimeLOAID = dtlett.Rows[dtlett.Rows.Count - 1]["LetterId"].ToString();
                                        dtBillTable.Rows[0]["HasDefaultLOA"] = 1;
                                        dtBillTable.AcceptChanges();
                                    }
                                }
                            }
                    ...
                    if (BillType == "1")
                    {
                        if (dtBillTable != null && dtBillTable.Rows.Count > 0)
                            dsCash.Tables.Add(dtBillTable.Copy());

                        #region Commented

                        if (dtDiscountDetails != null && dtDiscountDetails.Rows.Count > 0)
                        {
                            dtDiscountDetails.TableName = "Discounts";
                            int intRowcount = dtDiscountDetails.Rows.Count;
                            DataRow[] drDiscount = dtDiscountDetails.Select("TYP<>0");
                            dtDiscountDetails.Rows[intRowcount - 1].Delete();
                            dtDiscountDetails.AcceptChanges();
                            if (drDiscount.Length > 0)
                            {
                                DataTable dtZeroDisc = dtDiscountDetails.Clone();
                                DataRow drNew = null;
                                //foreach (DataRow drAdd in dtDiscountDetails.Rows)


                                drNew = dtZeroDisc.NewRow();
                                drNew["TYP"] = "0";
                                drNew["TOT"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([TOT])", ""));//drAdd["TOT"];
                                drNew["CPAY"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([CPAY])", "")); // drAdd["CPAY"];
                                drNew["PPAY"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([PPAY])", "")); //drAdd["PPAY"];
                                drNew["DCOM"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([DCOM])", "")); // drAdd["DCOM"];
                                drNew["DPAT"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([DPAT])", "")); // drAdd["DPAT"];
                                drNew["DIS"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([DIS])", "")); //drAdd["DIS"];
                                drNew["DPER"] = Convert.ToDecimal(dtDiscountDetails.Compute("sum([DPER])", "")); // drAdd["DPER"];
                                drNew["SEQ"] = Convert.ToInt32(dtDiscountDetails.Rows[0]["SEQ"].ToString());
                                drNew["AuthorisedBy"] = dtDiscountDetails.Rows[0]["AuthorisedBy"].ToString();
                                drNew["AuthorisedId"] = Convert.ToInt32(dtDiscountDetails.Rows[0]["AuthorisedId"].ToString());
                                drNew["Remarks"] = dtDiscountDetails.Rows[0]["Remarks"].ToString();
                                if (dtDiscountDetails.Rows[0]["LEVEL"] != "" && dtDiscountDetails.Rows[0]["LEVEL"] != null)
                                    drNew["LEVEL"] = Convert.ToInt32(dtDiscountDetails.Rows[0]["LEVEL"]);
                                else
                                    drNew["LEVEL"] = 0;
                                drNew["SID"] = "0";
                                drNew["DID"] = "0";
                                ...
                                drNew["QTY"] = "0";
                                drNew["BQTY"] = "0";
                                dtZeroDisc.Rows.Add(drNew);

                                dtZeroDisc.AcceptChanges();
                                foreach (DataRow drrow in dtZeroDisc.Rows)
                                {
                                    dtDiscountDetails.ImportRow(drrow);
                                }
                                dtDiscountDetails.AcceptChanges();
                            }

                            // end adding for contribution discount
                            dsCash.Tables.Add(dtDiscountDetails.Copy());
                        }
                        if (ddlservices != "28")
                        {
                            if (DtCompanyCashBLPackageItemDetails != null && DtCompanyCashBLPackageItemDetails.Rows.Count > 0)
                            {
                                DtCompanyCashBLPackageItemDetails.TableName = "CashPackage";
                                dsCash.Tables.Add(DtCompanyCashBLPackageItemDetails.Copy());
                            }
                        }
                        #endregion Commented
                        ...
                        if (intPackageID > 0)
                        {
                            if (dtTempLetter != null)
                            {
                                dtTempLetter.TableName = "Letter";
                                if (Convert.ToInt32(txtamount) > 0)
                                {
                                    dtTempLetter.Rows[0][0] = 0;
                                    dtTempLetter.AcceptChanges();
                                }
                                else
                                {
                                ...
                                dsCash.Tables.Add(dtTempLetter.Copy());
                            }
                        }
                        else
                        {
                        ...
                        dtMultiPayment = new DataTable();
                        if (dtSummaryTable != null)
                        {
                            dtMultiPayment = (DataTable)dtSummaryTable;
                            if (dtMultiPayment != null && dtMultiPayment.Rows.Count > 0)

                                // dtMultiPayment.TableName = "Payments";
                                foreach (DataRow dr in dtMultiPayment.Rows)
                                {
                                    if (dr["BANKID"].ToString() == "0")
                                        dr["BANKID"] = "";
                                    if (dr["CARDTYPEID"].ToString() == "0")
                                        dr["CARDTYPEID"] = "";
                                }
                            dtPayments = dtMultiPayment.Copy();
                            AssignPayments();
                            dtMultiPayment.TableName = "Payments";
                            dsCash.Tables.Add(dtMultiPayment.Copy());
                        ...
                        #region Service Patient Registration CR-55 for HCG--Naidu

                        //DataTable dtServicePatient = SavePatient(0);

                        // DataSet ds = new DataSet();
                        // ds.Tables.Add(dtServicePatient);

                        #endregion

                        if (dsCash.Tables["SaveBillDetails"].Columns.Contains("Cash"))
                            ActualPatientAmount = dsCash.Tables["SaveBillDetails"].Rows[0]["Cash"].ToString();


                        if (taxdiscount != null)
                        {
                            DataTable detaildis = (DataTable)taxdiscount.Copy();
                            detaildis.AcceptChanges();
                            detaildis = detaildis.Select("TYP=5").CopyToDataTable();
                            DataTable dtbilldtls = (DataTable)BillDetails.Copy();
                            dtbilldtls.AcceptChanges();

                            foreach (DataRow drvat in dtbilldtls.Select())
                            {
                                DataRow[] drcalc = detaildis.Select("SID=" + drvat["SID"] + " and SIID=" + drvat["SIID"] + " and SPID=" + drvat["SPID"]);
                                int itemqty = Convert.ToInt16(drcalc[0]["Quantity"]);
                                if (OPBAvailDisc != null & strBillType == "CS") itemqty = 1;
                                drvat["CVAT"] = Convert.ToDecimal(drcalc[0]["CVAT"]) / itemqty;
                                drvat["PVAT"] = Convert.ToDecimal(drcalc[0]["PVAT"]) / itemqty;
                            }
                            dtbilldtls.AcceptChanges();
                            dsCash.Tables.Add(dtbilldtls);
                        }
                        //end
                        if (POSTransatctionResult != null)
                        {
                            DataTable dtPOSResult = (DataTable)POSTransatctionResult.Copy(); dtPOSResult.AcceptChanges();
                            dsCash.Tables.Add(dtPOSResult);
                        }


                        StrCs = SaveBill(dsCash, strBillType.Trim(), "0").Split(Convert.ToChar("/"));


                        #region Service Patient Sequence Number

                        StrBLNO = StrCs[0].Trim();
                        strtoken = StrBLNO.Split(Convert.ToChar("-"));
                        //StringBuilder strbuilderBillNo = new StringBuilder();
                        if (hdnIsDirectPatient != "0")
                        {
                            StrBillId = Convert.ToString(StrCs[2].Trim());
                            hdnServicePatientNumber = StrCs[1].Trim().ToString();
                        }
                        #endregion
                        else
                        {
                            StrBillId = Convert.ToString(StrCs[1].Trim());
                        }
                        strbuilderBillNo.Append("Bill Number:" + strtoken[0] + "----");
                        strbuilderBillNo.Append("Generated Successfully");
                        BillNo = StrBillId;
                        if (strtoken.Length > 1)
                            TokenNO = strtoken[1].ToString();
                        else
                            TokenNO = null;

                        //System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','" + strbuilderBillNo.ToString() + "','OK','Information');", true);

                    }

                    else if (BillType == "2")
                    {
                        isBothBills = false;//(bool)DtCreditBillDetails.Rows[0]["IsBoth"];
                        //DataTable DtCompanyCreditBLItemDetails=;
                        DtCompanyCreditBLPackageItemDetails = DtCompanyCashBLPackageItemDetails.Copy();//Any time Only One ItemDetails will come
                        //if (PatientType == 1)//Commented by Shankar Addu for Dual Bill calculation for EMR/DYC/OUTP
                        //{
                        if (PatientType == 1 || PatientType == 3 || PatientType == 4)
                        {
                            #region Newly added for Dual bill calculation Dt:26-08-2011
                            //Below 2 DataTables added for Dual Bill Generation on 26-08-2011
                            if (ViewStatedtCompanyCashContribution != null)
                                dtTempCahsContribution = (DataTable)ViewStatedtCompanyCashContribution.Copy();
                            if (ViewStatedtCRContribution != null)
                                dtTempCreditContribution = (DataTable)ViewStatedtCRContribution.Copy();
                            if (dtTempCahsContribution != null && dtTempCahsContribution.Rows.Count > 0)
                            {
                                if (dtCompanyCashContribution != null && dtCompanyCashContribution.Rows.Count > 0)
                                {
                                    //dtCompanyCashContribution.Rows.Clear();
                                    dtCompanyCashContribution = dtTempCahsContribution.Copy();
                                }
                                if (dtCompanyCashContribution == null)
                                {
                                    //dtCompanyCashContribution.Rows.Clear();
                                    dtCompanyCashContribution = dtTempCahsContribution.Copy();
                                }
                            ...
                            //decimal dcTotalAvailDeposit = 0;
                            decimal decTextAmount = 0;
                            // dcTotalAvailDeposit = Convert.ToDecimal(gdvbillsummary.Rows[3].Cells[1].Text);
                            decimal dcTotalAvailDeposit = Convert.ToDecimal(BillSummary.Rows[4]["Amount"]);//171689-Lanka VAT CR-Akash Mishra/Mukesh Kumar-<31-JAN-2017>

                            string strTxtAmount = txtamount.Trim();


                            if (!string.IsNullOrEmpty(strTxtAmount))
                            {
                                decTextAmount = Convert.ToDecimal(strTxtAmount);
                            }

                            #endregion Newly added for Dual bill calculation Dt:26-08-2011

                            #region Multimode : bhaskar
                            DataTable dtCredPayment = CreatePaymentTable();
                            DataTable dtSupPayment = CreatePaymentTable();
                            DataTable dtTempMultiMode = CreatePaymentTable();
                            if (dtSummaryTable != null)
                            {
                                dtMultiPayment = (DataTable)dtSummaryTable;
                                dtTempMultiMode = dtMultiPayment.Clone();
                                foreach (DataRow drrow in dtMultiPayment.Rows)
                                {

                                    if (drrow["BANKID"].ToString() == "0")
                                        drrow["BANKID"] = "";
                                    if (drrow["CARDTYPEID"].ToString() == "0")
                                        drrow["CARDTYPEID"] = "";

                                    dtTempMultiMode.ImportRow(drrow);
                                }
                                dtTempMultiMode.AcceptChanges();
                            }
                            #endregion


                            if (dtCompanyCashContribution != null && dtCompanyCashContribution.Rows.Count > 0 && dtTempCahsContribution != null && dtTempCahsContribution.Rows.Count > 0) //SPAY > 0)//Newly added for SP bill with any amount including zero spay
                            {


                                if (dtBillTable != null && dtBillTable.Rows.Count > 0)
                                {
                                    //dsCompCash.Tables.Add(dtBillTable);//Commented on 26-08-2011 for dual bill
                                    #region Newly added for Dual bill generation dt: 26-08-2011
                                    DataTable dtSPBillTable = dtBillTable.Copy();



                                    decimal dcCollectable = 0;
                                    //dcCollectable = Convert.ToDecimal(gdvbillsummary.Rows[7].Cells[1].Text);
                                    if (BillSummary.Rows.Count > 8)
                                        dcCollectable = Convert.ToDecimal(BillSummary.Rows[8]["Amount"]);//171689-Lanka VAT CR-Akash Mishra/Mukesh Kumar-<31-JAN-2017>

                                    //decimal dcDeductable = Convert.ToDecimal(gdvbillsummary.Rows[8].Cells[1].Text);
                                    decimal dcGrandBalance = 0;
                                    decimal dcSpBalance = 0;
                                    #region Multimode : bhaskar
                                    //  if (chkmultimode.Checked)
                                    //   {
                                    if (dtSummaryTable != null)
                                    {
                                        if (dtMultiPayment != null && dtMultiPayment.Rows.Count > 0)
                                        //if (dtMultiPayment != null && dtMultiPayment.Rows.Count > 1)
                                        {
                                            decimal decsummaryamount = 0;

                                            decimal decsupamount = Convert.ToDecimal(dtBillTable.Rows[0]["SPAY"]);
                                            decsummaryamount = dcTotalAvailDeposit;
                                            // dtMultiPayment.TableName = "Payments";
                                            foreach (DataRow dr in dtMultiPayment.Rows)
                                            {
                                                decsummaryamount = decsummaryamount + Convert.ToDecimal(dr["amount"] == DBNull.Value ? 0 : dr["amount"]);
                                                if (decsummaryamount <= decsupamount)
                                                {
                                                    dtSupPayment.ImportRow(dr);
                                                    dtSupPayment.AcceptChanges();
                                                }
                                                else if ((Convert.ToDecimal(dtSupPayment.Compute("SUM(Amount)", "") == DBNull.Value ? 0 : dtSupPayment.Compute("SUM(Amount)", "")) + dcTotalAvailDeposit) < decsupamount)
                                                {
                                                    decimal decSupBalAmount = decsupamount - (Convert.ToDecimal(dtSupPayment.Compute("SUM(Amount)", "") == DBNull.Value ? 0 : dtSupPayment.Compute("SUM(Amount)", "")) + dcTotalAvailDeposit);
                                                    decimal decCreBalAmount = Convert.ToDecimal(dr["amount"] == DBNull.Value ? 0 : dr["amount"]) - decSupBalAmount;
                                                    dr["amount"] = decSupBalAmount;
                                                    dtSupPayment.ImportRow(dr);
                                                    dtSupPayment.AcceptChanges();
                                                    dr["amount"] = decCreBalAmount;
                                                    dtCredPayment.ImportRow(dr);
                                                    dtCredPayment.AcceptChanges();
                                                }
                                                else
                                                {
                                                    dtCredPayment.ImportRow(dr);
                                            ...
                                            dtPayments = dtSupPayment.Copy();
                                            AssignPayments();
                                            dtMultiPayment.TableName = "Payments";
                                            dsCompCash.Tables.Add(dtMultiPayment.Copy());
                                        }
                                        decTextAmount = Convert.ToDecimal(dtTempMultiMode.Compute("SUM(Amount)", "") == DBNull.Value ? 0 : dtTempMultiMode.Compute("SUM(Amount)", ""));
                                    }

                                    //   }
                                    #endregion
                                    if (dcTotalAvailDeposit == 0)
                                    {
                                        //dcGrandBalance = Convert.ToDecimal(gdvbillsummary.Rows[6].Cells[1].Text);
                                        dcGrandBalance = Convert.ToDecimal(BillSummary.Rows[7]["Amount"]);//171689-Lanka VAT CR-Akash Mishra/Mukesh Kumar-<31-JAN-2017>
                                        dcSpBalance = dcGrandBalance - (dcCollectable);

                                        if (dcGrandBalance - (dcCollectable + SPAY) > 0)
                                        {
                                            decimal dcexcessamt = (dcGrandBalance - (dcCollectable + SPAY));

                                            if (decTextAmount >= SPAY)
                                            {
                                                dtSPBillTable.Rows[0]["Cash"] = Convert.ToDecimal(dtBillTable.Rows[0]["SPAY"]);
                                                dtSPBillTable.Rows[0]["Credit"] = 0;
                                            }
                                            else
                                            {
                                                dtSPBillTable.Rows[0]["Cash"] = decTextAmount;
                                                dtSPBillTable.Rows[0]["Credit"] = SPAY - decTextAmount;
                                            }

                                        }
                                        else if (dcSpBalance - SPAY == 0)
                                        {
                                            if (decTextAmount >= SPAY)
                                            {
                                                dtSPBillTable.Rows[0]["Cash"] = Convert.ToDecimal(dtBillTable.Rows[0]["SPAY"]);
                                    ...
                                    else if (dcTotalAvailDeposit > 0)
                                    {
                                        dcGrandBalance = Convert.ToDecimal(BillSummary.Rows[7]["Amount"]);
                                        dcGrandBalance = dcGrandBalance + dcTotalAvailDeposit;
                                        dcSpBalance = dcGrandBalance - (dcCollectable);

                                        if (dcTotalAvailDeposit >= SPAY)
                                        {
                                            dtSPBillTable.Rows[0]["Cash"] = 0;
                                            dtSPBillTable.Rows[0]["Credit"] = 0;
                                            dtSPBillTable.Rows[0]["DepositAvailed"] = SPAY;

                                        }
                                        else
                                        {
                                            dtSPBillTable.Rows[0]["DepositAvailed"] = dcTotalAvailDeposit;
                                            if (decTextAmount > 0)
                                            {
                                                if (decTextAmount >= (SPAY - dcTotalAvailDeposit))
                                                {
                                                    dtSPBillTable.Rows[0]["Cash"] = (SPAY - dcTotalAvailDeposit);
                                                    dtSPBillTable.Rows[0]["Credit"] = 0;
                                                }
                                                else
                                                {
                                                    dtSPBillTable.Rows[0]["Cash"] = decTextAmount;
                                                    dtSPBillTable.Rows[0]["Credit"] = SPAY - dcTotalAvailDeposit - decTextAmount;
                                                }
                                            }
                                            else
                                            {
                                                dtSPBillTable.Rows[0]["Cash"] = 0;
                                                dtSPBillTable.Rows[0]["Credit"] = SPAY - dcTotalAvailDeposit;
                                            }
                                        }
                                    }

                                    dtSPBillTable.Rows[0]["CPAY"] = 0;
                                    dtSPBillTable.Rows[0]["PPAY"] = 0;
                                    dtSPBillTable.Rows[0]["DPAY"] = 0;

                                    dtSPBillTable.Rows[0]["Discount"] = 0;
                                    //dtSPBillTable.Rows[0]["Credit"] = 0;
                                    dtSPBillTable.Rows[0]["Amount"] = Convert.ToDecimal(dtBillTable.Rows[0]["SPAY"]);

                                    dtSPBillTable.Rows[0]["CVAT"] = Convert.ToDecimal("0");
                                    dtSPBillTable.Rows[0]["PVAT"] = Convert.ToDecimal(hdnPVATValueforSP);
                                    dtSPBillTable.AcceptChanges();
                                    dsCompCash.Tables.Add(dtSPBillTable);


                                    #endregion Newly added for Dual bill generation dt: 26-08-2011
                                }
...

                                {
                                    if (ddlservices != "28")
                                    {
                                        DtCompanyCreditBLPackageItemDetails.TableName = "CashPackage";

                                        dsCompCash.Tables.Add(DtCompanyCreditBLPackageItemDetails.Copy());
                                    }
                                }

                                if (intPackageID > 0)
                                {
                                    dsCompCash.Tables.Remove("CashPackage");
                                    DataTable dtOPPackageTbl = CreatePackageTbl();
                                    //  dtOPPackageTbl.TableName = "CreditPackage";
                                    dtOPPackageTbl.TableName = "CashPackage";
                                    dsCompCash.Tables.Add(dtOPPackageTbl.Copy());
                                }
                                ...
                                if (intPackageID > 0)
                                {
                                    ...
                                    if (dtTempLetter.Rows.Count > 0)
                                    {
                                        //dtTempLetter.Rows[0][0] = cmbOPPackages.SelectedValue;
                                        dtTempLetter.AcceptChanges();

                                    }

                                    dsCompCash.Tables.Add(dtTempLetter.Copy());
                                ...
                                else
                                {
                                    ...
                                    dsCompCash.Tables.Add(dtTempLetter.Copy());
                                    if (dtTempLetter.Rows.Count > 0)
                                        intDupLetterid = Convert.ToInt32(dtTempLetter.Rows[0][0]);
                                }



                                if (dsCompCash.Tables["SaveBillDetails"].Columns.Contains("Cash"))
                                    ActualPatientAmount = dsCompCash.Tables["SaveBillDetails"].Rows[0]["Cash"].ToString();

                                if (POSTransatctionResult != null)
                                {
                                    DataTable dtPOSResult = (DataTable)POSTransatctionResult.Copy(); dtPOSResult.AcceptChanges();
                                    dsCompCash.Tables.Add(dtPOSResult);
                                }

                                if (ConfigurationManager.AppSettings["SPBillRequired"] != null && ConfigurationManager.AppSettings["SPBillRequired"].ToString().ToUpper() == "NO")
                                {
                                    strtoken = new[] { "NOSP" };
                                    //System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','SP bill is not Allowed.','OK','Information');", true);
                                }
                                else
                                {

                                    StrCs = SaveBill(dsCompCash, "SP", "0").Split(Convert.ToChar("/"));
                            ...
                            #region newly added dual bill generation 26-08-2011
                            if (dtTempCreditContribution != null && dtTempCreditContribution.Rows.Count > 0)
                            {
                                if (dtCompanyCashContribution != null && dtCompanyCashContribution.Rows.Count > 0)
                                {
                                    //dtCompanyCashContribution.Rows.Clear();
                                    dtCompanyCashContribution = dtTempCreditContribution.Copy();
                                }
                                if (dtCompanyCashContribution == null)
                                {
                                    dtCompanyCashContribution = dtTempCreditContribution.Copy();
                            ...
                            #endregion newly added dual bill generation 26-08-2011

                            //modified by bhaskar: 19-8-2011 for credit patient zero amount

                            if (dtCompanyCashContribution != null && dtCompanyCashContribution.Rows.Count > 0 && dtTempCreditContribution != null && dtTempCreditContribution.Rows.Count > 0)//CPAY >= 0) commented on 26-08-2011 for dual bill generation
                            {
                                DataSet dsCompCredit = new DataSet();
                                //dsCompCredit.Tables.Add(dtBillTable.Copy());

                                #region Newly added for Dual Bill generation on dt:26-08-2011

                                decimal dcCollectable = Convert.ToDecimal(BillSummary.Rows[8]["Amount"]);
                                //decimal dcDeductable = Convert.ToDecimal(gdvbillsummary.Rows[8].Cells[1].Text);
                                decimal dcGrandBalance = 0;// Convert.ToDecimal(gdvbillsummary.Rows[6].Cells[1].Text);
                                decimal dcCpay = Convert.ToDecimal(BillSummary.Rows[1]["Amount"]);
                                decimal dcDiscount = Convert.ToDecimal(BillSummary.Rows[2]["Amount"]);
                                decimal dcSpBalance = 0;// dcGrandBalance - (dcCollectable + dcDeductable);

                                decimal dcTotalCPAY = dcCollectable + dcCpay + dcDiscount;
                                decimal dcExcess = 0;
                                decimal dcPPAY = 0;
                                #region Multimode : bhaskar
                                //    if (chkmultimode.Checked)
                                ...
                                if (dtSummaryTable != null)
                                {
                                    if (StrBillId.Length == 0)
                                    {
                                        dtTempMultiMode = (DataTable)dtSummaryTable;
                                        if (dtCredPayment != null && dtCredPayment.Rows.Count == 0)
                                        {
                                            if (dtTempMultiMode != null && dtTempMultiMode.Rows.Count > 0)
                                            {
                                                foreach (DataRow dr in dtTempMultiMode.Rows)
                                                {
                                ...
                                if (dcTotalAvailDeposit == 0)
                                {
                                    //dcGrandBalance = Convert.ToDecimal(gdvbillsummary.Rows[6].Cells[1].Text);
                                    //2017 175842-Hemas
                                    if (ConfigurationManager.AppSettings["TAXtype"] == null || Convert.ToInt32(ConfigurationManager.AppSettings["TAXtype"]) != 1)
                                    {
                                        dcGrandBalance = Convert.ToDecimal(BillSummary.Rows[6]["Amount"]);
                                    }
                                    else
                                    {
                                        dcGrandBalance = Convert.ToDecimal(BillSummary.Rows[7]["Amount"]);//171689-Lanka VAT CR-Akash Mishra/Mukesh Kumar-<31-JAN-2017>
                                    ...
                                    if (SPAY >= 0)
                                        dcExcess = dcSpBalance - SPAY;
                                    dcPPAY = dcCollectable + dcExcess;
                                    dcTotalCPAY = dcTotalCPAY + dcExcess;//dcTotalCPAY is nothing but CR bill amount


                                    dtBillTable.Rows[0]["Amount"] = dcTotalCPAY;// dtBillTable.Rows[0]["CPAY"];


                                    dtBillTable.Rows[0]["PPAY"] = dcPPAY;
                                    ...
                                    if (decTextAmount > 0)
                                    {
                                        if (decTextAmount > SPAY)
                                        {
                                            ...
                                            dtBillTable.Rows[0]["Cash"] = decTextAmount - SPAY;
                                            dtBillTable.Rows[0]["Credit"] = dcCpay;

                                        }
                                        else
                                        {
                                            dtBillTable.Rows[0]["Cash"] = 0;//already amount is adjusted for SP bill.
                                            dtBillTable.Rows[0]["Credit"] = dcCpay;
                                        }
                                    }
                                    else
                                ...
                                else if (dcTotalAvailDeposit > 0)
                                {
                                    ...
                                    dtBillTable.Rows[0]["Credit"] = dcCpay;
                                    decimal dcCRAvaildeposit = 0;
                                    decimal dcCRBal = 0;
                                    if (dcTotalAvailDeposit >= SPAY)
                                    {
                                        dcCRAvaildeposit = dcTotalAvailDeposit - SPAY;

                                    }
                                    dtBillTable.Rows[0]["DepositAvailed"] = dcCRAvaildeposit;
                                    dcCRBal = dcPPAY - dcCRAvaildeposit;
                                    if (decTextAmount >= dcCRBal)
                                    {
                                        dtBillTable.Rows[0]["Cash"] = dcCRBal;
                                    }
                                    else
                                    {
                                        dtBillTable.Rows[0]["Cash"] = decTextAmount; ;
                                    }
                                }
                                ...
                                //*** added by shankar for SP & CR bill checking
                                ...
                                if (dtCredPayment != null && dtCredPayment.Rows.Count > 0)
                                {
                                    dtPayments = dtCredPayment.Copy();
                                    AssignPayments();
                                    dtMultiPayment.TableName = "Payments";
                                    dsCompCredit.Tables.Add(dtMultiPayment.Copy());
                                }
                                //  }
                                #endregion
                                ...
                                {
                                    dtDiscountDetails.TableName = "Discounts";
                                    // added for Discount contribution : bhaskar on 26-11-2012
                                    int intRowcount = dtDiscountDetails.Rows.Count;
                                    DataRow[] drDiscount = dtDiscountDetails.Select("TYP<>0");
                                    dtDiscountDetails.Rows[intRowcount - 1].Delete();
                                    dtDiscountDetails.AcceptChanges();
                                    if (drDiscount.Length > 0)
                                    {
                                        ...
                                        DataTable dtZeroDisc = dtDiscountDetails.Clone();
                                        // DataTable dtItems = dtDepartment.Clone();
                                        DataRow drNew = null;
                                        //foreach (DataRow drAdd in dtDiscountDetails.Rows)
                                    ...
                                    // end adding for contribution discount 
                                    dsCompCredit.Tables.Add(dtDiscountDetails.Copy());
                                }
                                if (ddlservices != "28")
                                {
                                    if (DtCompanyCreditBLPackageItemDetails != null && DtCompanyCreditBLPackageItemDetails.Rows.Count > 0)
                                    {
                                        //commented by bhaskar : profile items for credit patient
                                        DtCompanyCreditBLPackageItemDetails.TableName = "CashPackage";
                                        //  DtCompanyCreditBLPackageItemDetails.TableName = "CreditPackage";
                                        dsCompCredit.Tables.Add(DtCompanyCreditBLPackageItemDetails.Copy());
                                    }
                                }
                                ...
                                {
                                    //commented by bhaskar : profile items for credit patient
                                    //   dsCompCredit.Tables.Remove("CreditPackage");
                                    if (dsCompCredit.Tables.Contains("CashPackage"))
                                        dsCompCredit.Tables.Remove("CashPackage");
                                    DataTable dtOPPackageTbl = CreatePackageTbl();
                                    //dtOPPackageTbl.TableName = "CreditPackage";
                                    dtOPPackageTbl.TableName = "CashPackage";

                                    dsCompCredit.Tables.Add(dtOPPackageTbl.Copy());
                                }
                                ...
                                if (intPackageID > 0)
                                {
                                    ...
                                    dsCompCredit.Tables.Add(dtTempLetter.Copy());
                                ...
                                {
                                    if (dsCompCredit.Tables["SaveBillDetails"].Columns.Contains("Cash") && ActualPatientAmount == null)
                                        ActualPatientAmount = dsCompCredit.Tables["SaveBillDetails"].Rows[0]["Cash"].ToString();
                                    if (DTTemp.Rows.Count > 0 & DTTemp.Select("serviceid=2").Length > 0)
                                    {
                                        dsCompCredit.Tables[0].Rows[0]["HasDefaultLOA"] = 1;
                                    }
                                    if (taxdiscount != null & BillDetails != null)
                                    {
                                        DataTable detaildis = (DataTable)taxdiscount;
                                        detaildis = detaildis.Select("TYP=5").CopyToDataTable();
                                        DataTable dtbilldtls = (DataTable)BillDetails;
                                        ...
                                        dtbilldtls.AcceptChanges();
                                        dsCompCredit.Tables.Add(dtbilldtls);
                                        dsCompCredit.Tables[dsCompCredit.Tables.Count - 1].TableName = "CreditBillItems";
                                    }
                                    //end
                                    if (POSTransatctionResult != null)
                                    {
                                        DataTable dtPOSResult = (DataTable)POSTransatctionResult.Copy(); dtPOSResult.AcceptChanges();
                                        dsCompCredit.Tables.Add(dtPOSResult);
                                    }

                                    try
                                    {
                                        StrCredit = SaveBill(dsCompCredit, "CR", "0").Split(Convert.ToChar("/"));
                                        StrBLNO = StrCredit[0].Trim();
                                        strtoken = StrBLNO.Split(Convert.ToChar("-"));
                                        if (hdnIsDirectPatient != "0")
                                        {
                                            StrBillId = Convert.ToString(StrCredit[2].Trim());
                                            hdnServicePatientNumber = StrCredit[1].Trim().ToString();
                                        }
                                        else
                                        {
                                            StrBillId = Convert.ToString(StrCredit[1].Trim());
                                        }
                                    ...
                                    {
                                        if (isFirstTimeLOA && !string.IsNullOrEmpty(strFirstTimeLOAID))
                                        {
                                            isFirstTimeLOA = false;
                                            Closer(strFirstTimeLOAID);
                                        }
                                        throw ex;
                                    }
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        StrCredit = SaveBill(dsCompCredit, "CR", StrCs[1].Trim().ToString()).Split(Convert.ToChar("/"));
                                        strBillCrNo = StrCs[0].Trim() + " & " + StrCredit[0].Trim();

                                        StrBLNO = strBillCrNo.ToString();
                                        StrBillId = Convert.ToString(StrCs[1].Trim());
                        ...
                        else  //if PatientType is 3, 4, 5 (Emergency, DayCare, OP Credit Issues from Pharmacy)
                        {
                            ...
                            DataSet dsCompCredit = new DataSet();
                            dsCompCredit.Tables.Add(dtBillTable.Copy());
                            //dsCompCredit.Tables.Add(DtCompanyCreditBLItemDetails.Copy());
                            if (isBothBills)
                            {

                            }
                            dsCompCredit = new DataSet();
                            dsCompCredit.Tables.Add(dtBillTable.Copy());
                    ...
                    //91969-Service WorkList --Bill
                    if (dtReferralOrders != null)
                    {
                        DataTable dtRefOrd = (DataTable)dtReferralOrders.Copy();
                        dtRefOrd.AcceptChanges();
                        if (dtRefOrd.Rows.Count > 0)
                        {
                            int BillConsID = 0; string strBillConsNo = string.Empty;
                            int ActionType = 1;
                            BillConsID = Convert.ToInt32(dtRefOrd.Rows[0]["BillableConsultationId"].ToString());
                            strBillConsNo = strtoken[0];
                            hdnDocID = dtRefOrd.Rows[0]["DoctorId"].ToString();
                            hdnDocSpecialiseId = dtRefOrd.Rows[0]["SpecialiseID"].ToString().Trim();
                            if (string.IsNullOrEmpty(hdnScheduleID))
                                hdnScheduleID = "0";
                            int id = SaveConsultationOrder(BillConsID, PatientId, Convert.ToInt32(hdnDocID), Convert.ToInt32(hdnDocSpecialiseId), strBillConsNo, ActionType, Convert.ToInt32(strDefaultUserId), Convert.ToInt32(strDefWorkstationId), 0, null, Convert.ToInt32(strDefaultHospitalId), Convert.ToInt32(hdnScheduleID));
                        }
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    DataRow drRow = dtSaveBillMeth.NewRow();
                    drRow["row"] = "Message : " + ex.Message.ToString() + " stack trace : " + ex.StackTrace.ToString();
                    dtSaveBillMeth.Rows.Add(drRow); dtSaveBillMeth.AcceptChanges();
                    dtSaveBillMeth.WriteXml(@"c:\\temp\\savebillmeth.xml");
                    //System.Web.UI.ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OutPatient Billing", "ShowMsgBox('" + this.Title + "','Exception:" + ex.Message.ToString() + "','OK','error');", true);

                }
                finally
                {

                    dtDiscountDetails = null;
                    dtBillTable = null;
                    dtTempLetter = null;
                    DTTemp = null;
                    dsCash = null;
                    dsCompCash = null;
                    hdnScheduleID = string.Empty;
        ...
        public int SaveConsultationOrder(int BillableConsultationID, int PatientId, int intRefDoctorID, int IntSpeclizationID, string strBillNo, int ActionType, int intUserID, int intWorkstationID, int intError, string strCallContext, int HospitalId, int ScheduleID)
        {
            ...
            int intRaiseOrderID = 0;

            try
            {
                intRaiseOrderID = objFOServiceClient.SaveConsultationOrder(BillableConsultationID, PatientId, intRefDoctorID, IntSpeclizationID, strBillNo, ActionType, intUserID, intWorkstationID, intError, strCallContext, HospitalId, ScheduleID);
            }
        ...
        private DataTable CreatePackageTbl()
        {
            DataTable dtOPPackageTbl = new DataTable();
            try
            {
                dtOPPackageTbl.Columns.Add("SEQ", typeof(int));
                dtOPPackageTbl.Columns.Add("SID", typeof(int));
                dtOPPackageTbl.Columns.Add("SIID", typeof(int));
                dtOPPackageTbl.Columns.Add("PID", typeof(int));
                dtOPPackageTbl.Columns.Add("SAID", typeof(int));
                dtOPPackageTbl.Columns.Add("SPID", typeof(int));
                dtOPPackageTbl.Columns.Add("OTYP", typeof(int));
                dtOPPackageTbl.Columns.Add("QTY", typeof(int));
                dtOPPackageTbl.Columns.Add("BQTY", typeof(int));
                dtOPPackageTbl.Columns.Add("URT", typeof(decimal));
                dtOPPackageTbl.Columns.Add("PRI", typeof(decimal));
                dtOPPackageTbl.Columns.Add("DIS", typeof(int));
                dtOPPackageTbl.Columns.Add("STID", typeof(int));
                dtOPPackageTbl.Columns.Add("CPAY", typeof(int));
                dtOPPackageTbl.Columns.Add("PPAY", typeof(int));
                dtOPPackageTbl.Columns.Add("OID", typeof(int));
                dtOPPackageTbl.Columns.Add("DPAY", typeof(int));
                dtOPPackageTbl.Columns.Add("TYP", typeof(int));
                dtOPPackageTbl.Columns.Add("ITMSEQ", typeof(int));
                dtOPPackageTbl.Columns.Add("DID", typeof(int));
                dtOPPackageTbl.Columns.Add("TOT", typeof(int));
                return dtOPPackageTbl;
            }
            catch (Exception ex)
            {
                //HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in CreatePackageTbl", "");
                return null;
            }
        }
        protected void btnMOPOK_Click()
        {
            try
            {
                decimal dAmount = Convert.ToDecimal(txtamount);
                BlanceAomunt = (Decimal)BAmount;
                decimal dOutStandAmount = Convert.ToDecimal(BlanceAomunt);
                BillPayAmount = BillPayAmount + Convert.ToDecimal(txtamount);
                intMOPoutstand = 0;
                int CardTypeID = 0;
                int ModeId = 0;
                int BankID = 0;
                ...
                Int64 VoucherNameID = 0;
                string VoucherName = string.Empty;
                string ReferenceNo = string.Empty;
                string CardHolderName = string.Empty;
                string EDCMachine = string.Empty;
                string Amount = "0";
                string ModeOfPayment = string.Empty;
                bool isvaliddate = true;
                decimal decBalance = 0;
                DataTable dtSummaryTableM = CreatePaymentTable();
                ModeId = 1;//Cash               
                string strValidity = Validity.ToString("dd-MMM-yyyy hh:mm:ss");
                string strTDATE = TDATE.ToString("dd-MMM-yyyy hh:mm:ss");
                string stramount = txtamount;
                Amount = Convert.ToDecimal(stramount).ToString(hdnsCurrencyFormat);

                dtSummaryTableM.Rows.Add(new object[] { ModeId, BankID,CardTypeID,ApprovalNo,Number,TDATE,Validity,VoucherNameID,
                VoucherName,CardHolderName,EDCMachine,Amount,ModeOfPayment,ReferenceNo});

                #region Multimode 
                if (dtSummaryTable != null)
                {
                    dtSummaryTableM = (DataTable)dtSummaryTable.Copy();
                    if (dtSummaryTableM.Rows.Count > 0)
                    {
                        ViewStateMultimode = "Multimode";
                        decBalance = Convert.ToDecimal(BillSummary.Rows[7]["Amount"]);
                        if (string.IsNullOrEmpty(TotAmount))
                        {
                            TotAmount = Amount;
                            txtamount = Convert.ToString(decBalance - Convert.ToDecimal(TotAmount));
                        }
                        if (dtSummaryTable != null)
                        {
                            dtSummaryTableM = (DataTable)dtSummaryTable.Copy();
                            dtSummaryTableM.AcceptChanges();
                            dtSummaryTableM.Rows.Add(new object[] { ModeId, BankID,CardTypeID,ApprovalNo,Number,strTDATE,strValidity,VoucherNameID,
                            VoucherName,CardHolderName,EDCMachine,Amount,ModeOfPayment,ReferenceNo});
                            TotAmount = Convert.ToDecimal(TotAmount.ToString()) + Convert.ToDecimal(Amount).ToString();
                            TotAmount = TotAmount.ToString();
                            txtamount = Convert.ToString(decBalance - Convert.ToDecimal(TotAmount));
                        }
                    }
                }
                ...
                #endregion
                dtSummaryTable = dtSummaryTableM.Copy();
                dtSummaryTable.AcceptChanges();
                gdvsummary = dtSummaryTableM.Copy();
                gdvsummary.AcceptChanges();
            }
            catch (Exception ex)
            {
                //HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in btnMOPOK_Click Event", "");
            }
        }
        private DataTable CreatePaymentTable()
        {
            DataTable dtSummaryTableMM = new DataTable("SaveDepositDetails");
            try
            {
                dtSummaryTableMM.Columns.Add("ModeId", typeof(string));
                dtSummaryTableMM.Columns.Add("BankID", typeof(string));
                dtSummaryTableMM.Columns.Add("CardTypeID", typeof(string));
                dtSummaryTableMM.Columns.Add("ApprovalNo", typeof(string));
                dtSummaryTableMM.Columns.Add("Number", typeof(string));
                dtSummaryTableMM.Columns.Add("TDATE", typeof(string));
                dtSummaryTableMM.Columns.Add("Validity", typeof(DateTime));
                dtSummaryTableMM.Columns.Add("VoucherNameID", typeof(string));
                dtSummaryTableMM.Columns.Add("VoucherName", typeof(string));
                dtSummaryTableMM.Columns.Add("CardHolderName", typeof(string));
                dtSummaryTableMM.Columns.Add("EDCMachine", typeof(string));
                dtSummaryTableMM.Columns.Add("Amount", typeof(decimal));
                dtSummaryTableMM.Columns.Add("ModeofPayment", typeof(string));
                dtSummaryTableMM.Columns.Add("ReferenceNo", typeof(string));
                return dtSummaryTableMM;
            }
            catch (Exception ex)
            {
                //HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in CreatePaymentTable", "");
                return null;
            }
        }
        ...
        private void FillAgreement(DataTable dtAgreementTemp)
        {

            int intPatientType = Convert.ToInt32(hdnPatientType);
            if (dtAgreementTemp.Rows.Count == 0)
            { return; }
            DataTable dtTempConfig = GetOriginalStructure(dtAgreementTemp.Copy());

            for (int intCnt = 0; intCnt < dtTempConfig.Rows.Count; intCnt++)
            {
                if ((dtTempConfig.Rows[intCnt]["LTID"].ToString() == "1") || (dtTempConfig.Rows[intCnt]["LTID"].ToString() == "3"))
                {
                    dtTempConfig.Rows[intCnt]["LTName"] = "No Limit";
                }
                else if ((dtTempConfig.Rows[intCnt]["LTID"].ToString() == "2") || (dtTempConfig.Rows[intCnt]["LTID"].ToString() == "4"))
                {
                    dtTempConfig.Rows[intCnt]["LTName"] = "Price Limit";
                }
                else if ((dtTempConfig.Rows[intCnt]["LTID"].ToString() == "5"))
                {
                    dtTempConfig.Rows[intCnt]["LTName"] = "Quantity";
                }
                if (dtTempConfig.Rows[intCnt]["LTName"].ToString().Trim() != "")
                { dtTempConfig.Rows[intCnt]["DisplayValue"] = dtTempConfig.Rows[intCnt]["LTName"].ToString() + " : " + Convert.ToDouble(dtTempConfig.Rows[intCnt]["LTValue"]).ToString(hdnsCurrencyFormat); }
                else
                { dtTempConfig.Rows[intCnt]["DisplayValue"] = ""; }

                string strParent = dtTempConfig.Rows[intCnt]["ParentID"].ToString();

                if (strParent.Trim().Length > 0)
                {
                    int MinusIndex = strParent.IndexOf("--1", 1);
                    int FirstIndex = 0, SecIndex = 0;
                    if (MinusIndex < 0 && intPatientType == 1)
                    { MinusIndex = MinusIndex; }
                    else if (MinusIndex < 0 && intPatientType == 2)
                    { MinusIndex = 0; }
                    if (MinusIndex < 0)
                    {
                        strParent = strParent.Remove(MinusIndex, 3);
                    }
                    else
                    {
                        FirstIndex = strParent.IndexOf("-", 1);
                        SecIndex = strParent.IndexOf("-", FirstIndex + 3);
                        if (SecIndex > 0)
                        { strParent = strParent.Remove(FirstIndex, SecIndex - FirstIndex); }
                        if (strParent == "BT" || SecIndex < 0) { strParent = "0"; }
                    }
                    dtTempConfig.Rows[intCnt]["ParentID"] = strParent;
                    string strTransID = dtTempConfig.Rows[intCnt]["TransID"].ToString();
                    MinusIndex = strTransID.IndexOf("--1", 1);
                    if (MinusIndex > 0)
                    {
                        strTransID = strTransID.Remove(MinusIndex, 3);
                    }
                    else
                    {
                        FirstIndex = strTransID.IndexOf("-", 1);
                        SecIndex = strTransID.IndexOf("-", FirstIndex + 1);
                        if (SecIndex > 0)
                        { strTransID = strTransID.Remove(FirstIndex, SecIndex - FirstIndex); }
                    }
                    dtTempConfig.Rows[intCnt]["TransID"] = strTransID;
                }
            }
            dtTempConfig.AcceptChanges();
            dtLetterSeventh = dtTempConfig.Copy();
        }
        private void SaveDefaultLOA(DataTable dtSeventhTable, DataTable dtGenTable, string StrBillNo, int intSpecializationID, string PackageName)
        {

            try
            {
                dtPatientDetails = (DataTable)dtPatientDetails.Copy();
                dtPatientDetails.AcceptChanges();
                DataTable dtBedTypes = new DataTable();
                dtBedTypes.Columns.Add("TID", typeof(string));
                dtBedTypes.Columns.Add("ID", typeof(string));
                dtBedTypes.Columns.Add("LMT", typeof(string));
                dtBedTypes.Columns.Add("LMV", typeof(string));
                dtBedTypes.Columns.Add("ISR", typeof(string));

                DataTable dtServices = new DataTable();
                dtServices.Columns.Add("TID", typeof(string));
                dtServices.Columns.Add("ID", typeof(string));
                dtServices.Columns.Add("LMT", typeof(string));
                dtServices.Columns.Add("LMV", typeof(string));
                dtServices.Columns.Add("ISR", typeof(string));
                dtServices.Columns.Add("PID", typeof(string));
                dtServices.Columns.Add("DisplayValue", typeof(string));
                dtServices.Columns.Add("ItemHOP", typeof(string));

                dtServices.Columns.Add("LAP", typeof(Int16));
                DataTable dtDepartment = dtServices.Clone();
                dtDepartment.Columns.Remove("ItemHOP");
                dtDepartment.Columns.Remove("LAP");
                dtDepartment.AcceptChanges();

                DataTable dtSpecialisations = dtDepartment.Clone();
                DataTable dtItems = dtDepartment.Clone();
                DataRow drNew = null;
                foreach (DataRow drAdd in dtSeventhTable.Rows)
                {
                    switch (drAdd["TransType"].ToString())
                    {
                        case "BT":
                            drNew = dtBedTypes.NewRow();
                            drNew["TID"] = -1;
                            drNew["ID"] = drAdd["TypeID"];
                            drNew["LMT"] = Convert.IsDBNull(drAdd["LTID"]) ? "" : (drAdd["LTID"]);
                            drNew["LMV"] = drAdd["LTValue"];
                            int LimitDays = Convert.ToInt32(drAdd["LTValue"]);
                            drNew["ISR"] = drAdd["ISR"];
                            dtBedTypes.Rows.Add(drNew);
                            break;
                        case "ST":
                            drNew = dtServices.NewRow();
                            drNew["TID"] = drAdd["TransID"];
                            drNew["ID"] = drAdd["TypeID"];
                            drNew["LMT"] = drAdd["LTID"];
                            drNew["LMV"] = drAdd["LTValue"];
                            drNew["ISR"] = drAdd["ISR"];
                            drNew["PID"] = drAdd["ParentID"];
                            drNew["DisplayValue"] = drAdd["LTName"] + ":" + drAdd["LTValue"];
                            drNew["LAP"] = drAdd["LimitApplicable"];
                            drNew["ItemHOP"] = drAdd["ItemHOP"];
                            dtServices.Rows.Add(drNew);
                            break;
                        case "DT":
                            drNew = dtDepartment.NewRow();
                            drNew["TID"] = drAdd["TransID"];
                            drNew["ID"] = drAdd["TypeID"];
                            drNew["LMT"] = drAdd["LTID"];
                            drNew["LMV"] = drAdd["LTValue"];
                            ...
                            drNew["PID"] = drAdd["ParentID"];
                            drNew["DisplayValue"] = drAdd["LTName"] + ":" + drAdd["LTValue"];
                            dtDepartment.Rows.Add(drNew);
                            break;
                        case "DP":
                            drNew = dtSpecialisations.NewRow();
                            drNew["TID"] = drAdd["TransID"];
                            drNew["ID"] = drAdd["TypeID"];
                            drNew["LMT"] = drAdd["LTID"];
                            drNew["LMV"] = drAdd["LTValue"];
                            ...
                            drNew["PID"] = drAdd["ParentID"];
                            drNew["DisplayValue"] = drAdd["LTName"] + ":" + drAdd["LTValue"];
                            dtSpecialisations.Rows.Add(drNew);
                            break;
                        case "SP":
                ...
                dtBedTypes.AcceptChanges();
                dtServices.AcceptChanges();
                dtDepartment.AcceptChanges();
                dtSpecialisations.AcceptChanges();
                dtItems.AcceptChanges();

                DataTable dtGenInfo = new DataTable("SaveLOA");
                dtGenInfo.Columns.Add("LetterID", typeof(string));
                dtGenInfo.Columns.Add("PatientID", typeof(int));
                dtGenInfo.Columns.Add("CustomerID", typeof(int));
                dtGenInfo.Columns.Add("LetterNo", typeof(string));
                dtGenInfo.Columns.Add("FromDate", typeof(string));
                dtGenInfo.Columns.Add("ToDate", typeof(string));
                dtGenInfo.Columns.Add("LetterDate", typeof(string));
                dtGenInfo.Columns.Add("PatientType", typeof(int));
                dtGenInfo.Columns.Add("AuthorisedBy", typeof(string));
                dtGenInfo.Columns.Add("Blocked", typeof(bool));
                dtGenInfo.Columns.Add("ReferenceNo", typeof(string));
                dtGenInfo.Columns.Add("InsuranceCompany", typeof(string));
                dtGenInfo.Columns.Add("EmployeeName", typeof(string));
                dtGenInfo.Columns.Add("PolicyNo", typeof(string));
                dtGenInfo.Columns.Add("BasicPay", typeof(decimal));
                dtGenInfo.Columns.Add("PolicyIssueDate", typeof(string));
                dtGenInfo.Columns.Add("PolicyExpiryDate", typeof(string));
                dtGenInfo.Columns.Add("RelationshipID", typeof(int));
                dtGenInfo.Columns.Add("InssuredAmount", typeof(long));
                dtGenInfo.Columns.Add("CardCollectable", typeof(bool));
                dtGenInfo.Columns.Add("ApprovalTypeID", typeof(int));

                dtGenInfo.Columns.Add("InsuranceNo", typeof(string));
                dtGenInfo.Columns.Add("RelationCode", typeof(string));


                dtGenInfo.Columns.Add("GradeID", typeof(int));
                dtGenInfo.Columns.Add("DoctorID", typeof(int));
                dtGenInfo.Columns.Add("SpecialisationID", typeof(int));
                dtGenInfo.Columns.Add("NoOfFollwUps", typeof(int));

                dtGenInfo.Columns.Add("Featureid", typeof(int));
                dtGenInfo.Columns.Add("Functionid", typeof(int));
                dtGenInfo.Columns.Add("CallContext", typeof(string));
                dtGenInfo.Columns.Add("ParentLetterId", typeof(int));

                dtGenInfo.Columns.Add("LetterNo2l", typeof(string));
                dtGenInfo.Columns.Add("AuthorisedBy2l", typeof(string));
                dtGenInfo.Columns.Add("EmployeeName2l", typeof(string));


                DataRow drAddSave = dtGenInfo.NewRow();
                drAddSave["LetterID"] = 0;
                if (!string.IsNullOrEmpty(hdnPatientID))
                    drAddSave["PatientID"] = Convert.ToInt32(hdnPatientID);
                if (string.IsNullOrEmpty(strCompanyID))
                {
                    drAddSave["CustomerID"] = -1;//2455 ;//Convert.ToInt32(txtCompany.Tag);
                }
                else
                {
                    drAddSave["CustomerID"] = Convert.ToInt32(strCompanyID);
                }
                System.TimeSpan ts = Convert.ToDateTime(dtGenTable.Rows[0]["ToDate"].ToString()) - Convert.ToDateTime(dtGenTable.Rows[0]["FromDate"].ToString());
                int LmtDays = Convert.ToInt32(ts.Days.ToString());
                if (LmtDays == 0)
                {
                    LmtDays = 1;
                }
                ...
                drAddSave["LetterNo"] = PackageName + "-" + StrBillNo;
                string strSecond = (System.DateTime.Now).ToString("dd-MMM-yyyy hh:mm tt");
                string strDays = (System.DateTime.Now.AddDays((double)LmtDays).ToString("dd-MMM-yyyy hh:mm tt"));
                string strLetterDate = (System.DateTime.Now.Date.ToString("dd-MMM-yyyy hh:mm tt"));
                if (!string.IsNullOrEmpty(CONSFollowupDAYS.ToString()))
                    strLetterDate = (System.DateTime.Now.Date.AddDays(Convert.ToInt16(CONSFollowupDAYS) + 1).ToString("dd-MMM-yyyy hh:mm tt"));


                drAddSave["FromDate"] = strSecond;
                drAddSave["ToDate"] = strDays;
                drAddSave["LetterDate"] = strLetterDate;
                drAddSave["PatientType"] = 1;
                drAddSave["AuthorisedBy"] = dtGenTable.Rows[0]["AuthorisedBy"].ToString();
                drAddSave["Blocked"] = false;
                if (dtPatientDetails.Rows.Count > 0)
                {

                    if (dtPatientDetails.Columns.Contains("PolicyNo"))
                        drAddSave["ReferenceNo"] = dtPatientDetails.Rows[0]["PolicyNo"].ToString();
                    else
                        drAddSave["ReferenceNo"] = 0;

                    drAddSave["InsuranceCompany"] = txtpayer.ToString();
                    drAddSave["EmployeeName"] = dtPatientDetails.Rows[0]["PatientEmpId"].ToString();

                    if (dtPatientDetails.Columns.Contains("ContractNo"))
                        drAddSave["PolicyNo"] = dtPatientDetails.Rows[0]["ContractNo"].ToString();
                    else
                        drAddSave["PolicyNo"] = 0;

                    drAddSave["BasicPay"] = dtGenTable.Rows[0]["BasicPay"].ToString();

                    if (dtPatientDetails.Columns.Contains("PolicyValidFrom") && dtPatientDetails.Rows[0]["PolicyValidFrom"] != DBNull.Value)
                    {
                        drAddSave["PolicyIssueDate"] = Convert.ToDateTime(dtPatientDetails.Rows[0]["PolicyValidFrom"].ToString()).ToString("dd-MMM-yyyy");
                    }
                    else
                        drAddSave["PolicyIssueDate"] = DateTime.Now.ToString("dd-MMM-yyyy");

                    if (dtPatientDetails.Columns.Contains("PolicyValidTo") && dtPatientDetails.Rows[0]["PolicyValidTo"] != DBNull.Value)
                        drAddSave["PolicyExpiryDate"] = Convert.ToDateTime(dtPatientDetails.Rows[0]["PolicyValidTo"].ToString()).ToString("dd-MMM-yyyy");
                    else
                        drAddSave["PolicyExpiryDate"] = DateTime.Now.ToString("dd-MMM-yyyy");

                    if (dtPatientDetails.Columns.Contains("EmployeeRelationID"))
                        drAddSave["RelationshipID"] = Convert.IsDBNull(dtPatientDetails.Rows[0]["EmployeeRelationID"]) ? 0 : Convert.ToInt32(dtPatientDetails.Rows[0]["EmployeeRelationID"]);
                    else
                        drAddSave["RelationshipID"] = 0;


                    if (dtPatientDetails.Columns.Contains("PolicyNo"))
                        drAddSave["InsuranceNo"] = Convert.IsDBNull(dtPatientDetails.Rows[0]["PolicyNo"]) ? "" : dtPatientDetails.Rows[0]["PolicyNo"].ToString();
                    else
                        drAddSave["InsuranceNo"] = 0;


                    drAddSave["RelationCode"] = Convert.IsDBNull(dtPatientDetails.Rows[0]["RelationCode"]) ? "" : dtPatientDetails.Rows[0]["RelationCode"].ToString();

                }
                else
                {
                    drAddSave["ReferenceNo"] = dtGenTable.Rows[0]["ReferenceNo"].ToString();
                    drAddSave["InsuranceCompany"] = txtpayer.ToString();
                    drAddSave["EmployeeName"] = dtGenTable.Rows[0]["EmployeeName"].ToString();
                    drAddSave["PolicyNo"] = dtGenTable.Rows[0]["PolicyNo"].ToString();
                    drAddSave["BasicPay"] = dtGenTable.Rows[0]["BasicPay"].ToString();
                    drAddSave["PolicyIssueDate"] = Convert.IsDBNull(dtGenTable.Rows[0]["PolicyIssueDate"]) ? System.DateTime.Now.ToString("dd-MMM-yyyy") : Convert.ToDateTime(dtGenTable.Rows[0]["PolicyIssueDate"].ToString()).ToString("dd-MMM-yyyy");//Convert.ToDateTime(dtGenTable.Rows[0]["PolicyIssueDate"].ToString()).ToString("dd-MMM-yyyy");
                    drAddSave["PolicyExpiryDate"] = Convert.IsDBNull(dtGenTable.Rows[0]["PolicyExpiryDate"]) ? System.DateTime.Now.ToString("dd-MMM-yyyy") : Convert.ToDateTime(dtGenTable.Rows[0]["PolicyExpiryDate"].ToString()).ToString("dd-MMM-yyyy");//Convert.ToDateTime(dtGenTable.Rows[0]["PolicyExpiryDate"].ToString()).ToString("dd-MMM-yyyy");
                    drAddSave["RelationshipID"] = "1";
                }
                drAddSave["InssuredAmount"] = 0;
                drAddSave["ApprovalTypeID"] = 1;//dtGenTable.Rows[0]["ApprovalTypeID"].ToString();
                drAddSave["CardCollectable"] = 0;//Convert.ToBoolean(dtGenTable.Rows[0]["CardCollectable"]);
                if (strGradeID == "" || Convert.ToInt32(strGradeID) == 0)
                {
                    drAddSave["GradeID"] = 1;//DBNull.Value ;//Convert.ToInt32(txtGrade.Tag) ;
                }
                else
                {
                    drAddSave["GradeID"] = Convert.ToInt32(strGradeID);
                }

                if (!string.IsNullOrEmpty(hdnDocID))
                    drAddSave["DoctorID"] = Convert.ToInt32(hdnDocID);
                else if (!string.IsNullOrEmpty(hdnintBillDocEmpId))
                    drAddSave["DoctorID"] = Convert.ToInt32(hdnintBillDocEmpId);


                drAddSave["SpecialisationID"] = intSpecializationID;
                drAddSave["NoOfFollwUps"] = Convert.IsDBNull(dtGenTable.Rows[0]["NoOfFollwUps"]) ? 0 : Convert.ToInt32(dtGenTable.Rows[0]["NoOfFollwUps"]);

                drAddSave["Featureid"] = 391;
                drAddSave["Functionid"] = 2;
                drAddSave["CallContext"] = "SaveLOA";
                drAddSave["ParentLetterId"] = 0;

                dtGenInfo.Rows.Add(drAddSave);


                DataSet dsSaveLOA = new DataSet();
                dsSaveLOA.Tables.Add(dtBedTypes.Copy());
                dsSaveLOA.Tables.Add(dtServices.Copy());
                dsSaveLOA.Tables.Add(dtDepartment.Copy());
                dsSaveLOA.Tables.Add(dtSpecialisations.Copy());
                dsSaveLOA.Tables.Add(dtItems.Copy());
                dsSaveLOA.Tables.Add(dtGenInfo.Copy());

                dsSaveLOA.AcceptChanges();

                int HospitalID = Convert.ToInt32(strDefaultHospitalId);
                bool blnSavePackageLOA = SaveLOA(dsSaveLOA, 0, 0, 0, HospitalID);
            }
            finally
            {
        ...
        private void AssignPayments()
        {
            ...
            dtMultiPayment.Columns.Add("VID", System.Type.GetType("System.Int64"));
            ...
            foreach (DataRow drPay in dtPayments.Rows)
            {
                DataRow drMulti = dtMultiPayment.NewRow();
                drMulti["PMID"] = drPay["ModeId"];
                drMulti["CID"] = drPay["CardTypeID"].ToString() == "" ? DBNull.Value : drPay["CardTypeID"];
                drMulti["BID"] = drPay["BankID"].ToString() == "" ? DBNull.Value : drPay["BankID"];
                drMulti["EDATE"] = Edate.ToString("dd-MMM-yyyy");//DateTime.Now;//changed by babu
                drMulti["EID"] = DBNull.Value;
                drMulti["ET"] = "RC";
                drMulti["AMT"] = drPay["Amount"];
                drMulti["TNO"] = drPay["ApprovalNo"] == DBNull.Value ? 0 : drPay["ApprovalNo"];
                drMulti["CNO"] = drPay["Number"] == DBNull.Value ? " " : drPay["Number"];
                if (drPay["TDATE"] != DBNull.Value)
                    drMulti["TDATE"] = Convert.ToDateTime(drPay["TDATE"]).ToString("dd-MMM-yyyy");//dd-MM-yyyy
                else
                    drMulti["TDATE"] = DBNull.Value; //drPay["TDATE"];

                if (drPay["Validity"] != DBNull.Value)
                    drMulti["VTO"] = Convert.ToDateTime(drPay["Validity"]).ToString("dd-MMM-yyyy");
                else
                    drMulti["VTO"] = DBNull.Value;//drPay["Validity"];
                drMulti["CON"] = "";// txtpaidto.Text == "" ? " " : txtpaidto.Text;
                drMulti["REM"] = " ";
                drMulti["ORINT"] = 0;
                drMulti["CTYP"] = 0;
                drMulti["VID"] = drPay["VoucherNameID"];
                drMulti["VNAME"] = drPay["VoucherName"];
                drMulti["CHOL"] = drPay["CardHolderName"];
                drMulti["EDCM"] = drPay["EDCMachine"];
                dtMultiPayment.Rows.Add(drMulti);
                cash = cash + Convert.ToDecimal(drPay["Amount"]);
            }
        }
        private void Closer(string strLOAID)
        {
            DataSet dsCloser = new DataSet();
            DataTable dtCloser = new DataTable();
            DataTable dtCloserXML = new DataTable();
            DataRow drAdd = null;
            string strCloseXML = string.Empty;
            try
            {
                dtCloserXML.Columns.Add("LID", typeof(int));

                DataTable dtClosers = TempTableCloser();
                DataRow drrow;
                drrow = dtCloserXML.NewRow();

                drrow["LID"] = strLOAID;
                dtCloserXML.Rows.Add(drrow);
                dtCloserXML.AcceptChanges();


                strCloseXML = Utilities.ConvertDTToXML("PP", "PL", dtCloserXML);
                dtCloser.Columns.Add("LetterText", typeof(string));
                dtCloser.Columns.Add("Featureid", typeof(Int16));
                dtCloser.Columns.Add("Functionid", typeof(Int16));
                dtCloser.Columns.Add("CallContext", typeof(string));
                dtCloser.Columns.Add("USERID", typeof(Int16));
                dtCloser.Columns.Add("WORKSTATIONID", typeof(Int16));
                dtCloser.Columns.Add("Error", typeof(Int16));

                drAdd = dtCloser.NewRow();
                drAdd["LetterText"] = strCloseXML;
                drAdd["Featureid"] = 1189;
                drAdd["Functionid"] = 2;
                drAdd["CallContext"] = "LOA CLOSER";
                drAdd["USERID"] = Convert.ToInt32(strDefaultUserId);
                drAdd["WORKSTATIONID"] = Convert.ToInt16(strDefWorkstationId);
                drAdd["Error"] = 0;
                dtCloser.Rows.Add(drAdd);
                dtCloser.AcceptChanges();

                dsCloser.Tables.Add(dtCloser);
                dsCloser.AcceptChanges();
                bool blnFlag = LOAForceCloser(dsCloser);
            }
            catch (Exception ex)
            {
                //HIS.TOOLS.Logger.ErrorLog.LogToXMLFile = false;
                //HIS.TOOLS.Logger.ErrorLog.ErrorRoutine(false, ex, "OP Billing", "Exception in OP Bill -> Closer() method", "OutPatientBilling.aspx.cs");
            }
            finally
            {
        ...
        public string SaveBill(DataSet dsBill, string BillPrefix, string ParentBillId)
        {
            try
            {
                objFOClient = new FrontOfficeServiceContractClient();
                return objFOClient.SaveBill(dsBill, BillPrefix, ParentBillId);
            }
        ...
        private DataTable GetOriginalStructure(DataTable dtSend)
        {
            try
            {
                DataTable dtTemp = dtSend.Clone();
                dtTemp.Columns.Remove("ItemHop");
                dtTemp.AcceptChanges();

                if (!dtTemp.Columns.Contains("DisplayValue"))
                {
                    dtTemp.Columns.Add("DisplayValue", typeof(string));
                }
                if (!dtTemp.Columns.Contains("ItemHop"))
                {
                    dtTemp.Columns.Add("ItemHop", typeof(string));
                }
                if (!dtTemp.Columns.Contains("FLEVEL"))
                {
                    dtTemp.Columns.Add("FLEVEL", typeof(bool));
                }

                foreach (DataRow drAdd in dtSend.Rows)
                {
                    dtTemp.ImportRow(drAdd);
                }
                dtTemp.AcceptChanges();
                return dtTemp;
            }
            ...
            {
                //mHIS.TOOLS.Logger.ErrorLog.ErrorRoutine(ex, MODULE_NAME, "Error in GetOriginalStructure", "");
                return null;
            }
        }
        public bool SaveLOA(DataSet dsSaveLOA, int intUserID, int intWorkstationid, int intError, int HospitalID)
        {
            ContractMgmtServiceContractClient objContractMgtClient = new ContractMgmtServiceContractClient();
            try
            {
                return objContractMgtClient.SaveHospitalLOA(dsSaveLOA, intUserID, intWorkstationid, intError, HospitalID);
            }
        ...
        public bool LOAForceCloser(DataSet dsLOACloser)
        {
            ContractMgmtServiceContractClient objLOAForceCloser = new ContractMgmtServiceContractClient();
            try
            {
                Boolean blnLOAForceCloser = new Boolean();
                blnLOAForceCloser = objLOAForceCloser.LOAForceCloser(dsLOACloser);
                return blnLOAForceCloser;
            }
            finally
            {
                objLOAForceCloser.Close();
            }

        }
        private DataTable TempTableCloser()
        {
            DataTable dtShowLOAs = new DataTable();
            //dtShowLOAs.Columns.Add("LetterID", typeof(int));
            dtShowLOAs.Columns.Add("Close", typeof(bool));
            dtShowLOAs.Columns.Add("LetterNo", typeof(string));
            dtShowLOAs.Columns.Add("LetterDate", typeof(string));
            dtShowLOAs.Columns.Add("UHID", typeof(string));
            //dtShowLOAs.Columns.Add("PatientID", typeof(int));
            dtShowLOAs.Columns.Add("PatientName", typeof(string));
            //dtShowLOAs.Columns.Add("PayerID", typeof(int));
            dtShowLOAs.Columns.Add("PayerName", typeof(string));
            dtShowLOAs.Columns.Add("ValidFrom", typeof(string));
            dtShowLOAs.Columns.Add("ValidTo", typeof(string));
